...
// 당근 SEED - 런타임 패키지(완성 컴포넌트 + 레이아웃 프리미티브)
import {
  ActionButton,
  Icon,
  Badge as SeedBadge,
...
import { Avatar as SeedAvatar } from 'seed-design/ui/avatar';
import { SegmentedControl, SegmentedControlItem } from 'seed-design/ui/segmented-control';
// 당근 SEED - 모노크롬 아이콘
// ⚠️ 반드시 named import 로만 가져온다. 네임스페이스 통째로(와일드카드) 받으면
//    아이콘 676개가 전부 번들에 들어간다.
import {
  ...
  IconPictureLine,
  IconCouponLine,
  IconCheckmarkBadgeLine,
  IconCupHeatwaveLine,
  IconSparkle2Line,
  IconMusicalnoteDoubleLine,
  IconCalendarLine,
  IconFlameLine,
  IconLinechartUpXaxisLine,
  IconGridLine,
  IconBookOpenLine,
  IconBookOpenFill,
...
// icons.jsx - 아이콘 어댑터
//
// 아이콘은 기본적으로 SEED 모노크롬 세트로 렌더한다(아래 SEED_ICONS). 자체 SVG path
// (ICON_PATHS)로 남긴 것은 12종뿐이다.
//
//   - SEED 에 대응물이 없다 (8종): mountain · waves · droplets · trees ·
//     mountain-snow · wind · church · lamp. 당근은 로컬커머스 서비스라
//     지형·자연물·종교시설 아이콘을 만들지 않는다
//   - 대체 시 의미 손실이 커서 유지 (4종): landmark(유적 계열 대표. SEED 는 현대
//     빌딩뿐이라 고전 기둥 건물 뉘앙스가 사라진다) · library(문화 계열 대표.
//     책장 vs 책 1권) · sunset(일몰 뉘앙스 소실) · message-circle(원형 말풍선 없음)

// 우리 아이콘 이름 → SEED 모노크롬 컴포넌트 [Line, Fill?]
// Fill 변형은 필요한 곳(탭바 활성 표현)만 넣는다. 안 쓰는 걸 넣으면 번들만 커진다.
const SEED_ICONS = {
  search:          [IconMagnifyingglassLine],
  bell:            [IconBellLine],
  bookmark:        [IconBookmarkLine],
  check:           [IconCheckmarkLine],
  close:           [IconXmarkLine],
  'chevron-down':  [IconChevronDownLine],
  'chevron-up':    [IconChevronUpLine],
  'chevron-left':  [IconChevronLeftLine],
  'chevron-right': [IconChevronRightLine],
  plus:            [IconPlusLine],
  heart:           [IconHeartLine],
  'arrow-right':   [IconArrowRightLine],
  star:            [IconStarLine],
  settings:        [IconGearLine],
  user:            [IconPersonLine, IconPersonFill],
  users:           [IconPerson2Line, IconPerson2Fill],
  'map-pin':       [IconLocationpinLine],
  map:             [IconMapLine],
  compass:         [IconCompassLine, IconCompassFill],
  navigation:      [IconPaperplaneLine],
  leaf:            [IconLeafLine],
  home:            [IconHouseLine, IconHouseFill],
  flag:            [IconFlagLine],
  archive:         [IconBoxFlapLine],
  image:           [IconPictureLine],
  ticket:          [IconCouponLine],
  award:           [IconCheckmarkBadgeLine],
  coffee:          [IconCupHeatwaveLine],
  sparkles:        [IconSparkle2Line],
  music:           [IconMusicalnoteDoubleLine],
  calendar:        [IconCalendarLine],
  flame:           [IconFlameLine],
  'trending-up':   [IconLinechartUpXaxisLine],
  grid:            [IconGridLine],
  'book-open':     [IconBookOpenLine, IconBookOpenFill],
  external:        [IconArrowUpRightLine],
  refresh:         [IconArrow2ClockwiseCircularLine],
  gift:            [IconGiftLine],
  globe:           [IconGlobeLine],
  info:            [IconILowercaseSerifCircleLine],
  trophy:          [IconTrophyLine],
};

type IconName = keyof typeof ICON_PATHS | keyof typeof SEED_ICONS;

interface SeedCSSProperties extends CSSProperties {
  [property: `--seed-${string}`]: string | number | undefined;
}
...
// 아이콘 어댑터. 호출부 시그니처(`<I n s c w fill />`)를 그대로 유지한 채
// SEED 아이콘과 자체 path 를 한 컴포넌트 뒤로 숨긴다.
//
// ⚠️ SEED 아이콘 경로에서는 `w`(선 굵기)가 무시된다. SEED 모노크롬 아이콘은 stroke 가
//    아니라 `fill="currentColor"` 아웃라인 도형이라 선 굵기가 도형에 구워져 있고,
//    바깥에서 조절할 방법이 없다. `w` 를 아무리 바꿔도 SEED_ICONS 에 있는 이름은
//    두께가 변하지 않는다. 굵기로 강조하고 싶으면 Line ↔ Fill 스왑을 써라
//    (`fill` 을 'none' 이 아닌 값으로 주면 Fill 변형이 있는 아이콘은 Fill 로 렌더된다).
//    `w` 는 아래 ICON_PATHS 12종에만 적용된다.
const I = React.forwardRef<SVGSVGElement, IconProps>(function I(
  { n, s = 16, c = 'currentColor', w = 1.75, fill = 'none', style, ...rest }, ref,
) {
  // 아이콘은 기본적으로 장식이므로 접근성 트리에서 숨긴다. 아이콘 자체가 의미를
  // 나르는 자리에서는 호출부가 role="img" + aria-label 을 넘겨 이 기본값을 덮는다.
  const a11y: SVGProps<SVGSVGElement> =
    (rest['aria-label'] || rest['aria-labelledby'] || rest.role)
      ? {}
      : { 'aria-hidden': true };
  const seed = n in SEED_ICONS
    ? SEED_ICONS[n as keyof typeof SEED_ICONS]
    : undefined;
  if (seed) {
    const C = (fill !== 'none' && seed[1]) ? seed[1] : seed[0];
    // SEED 아이콘의 path 는 fill="currentColor" 다. 색은 CSS color 로 준다.
    return <C ref={ref} size={s} style={{ flexShrink: 0, color: c, ...style }} {...a11y} {...rest} />;
  }
  const d = n in ICON_PATHS
    ? ICON_PATHS[n as keyof typeof ICON_PATHS]
    : undefined;
  if (!d) return null;
  ...
  return (
    <svg
      ...
      /* stroke·fill 을 CSS 속성으로도 준다. presentation attribute 는 var() 해석이
         브라우저마다 달라서, 토큰(var(--seed-color-*))을 c/fill 로 받으면 속성만으로는
         색이 적용되지 않을 수 있다. ...style 이 뒤라 호출부 override 는 그대로 유지. */
      style={{ flexShrink: 0, display: 'inline-block', stroke: c, fill, ...style }}
    >
...
// button 이 아닌 엘리먼트(<div>·SVG <g>)를 클릭 대상으로 쓸 때 키보드 접근성을 부여한다.
// <div {...clickable(fn)}> 형태로 펼쳐 쓴다. <button> 으로 바꿀 수 있는 자리라면 그게 낫지만,
// SVG <g> 나 이미 button 을 자식으로 품은 카드는 치환할 수 없어 이 헬퍼로 처리한다.
...
function clickable(onActivate: () => void): ClickableProps {
  return {
    role: 'button',
    tabIndex: 0,
    onClick: () => onActivate(),
...
// | 우리 프리미티브 | SEED 대응                       | 채널        |
// |---|---|---|
// | Button      | ActionButton                    | 런타임 |
// | Badge       | Badge                           | 런타임 |
// | Chip        | Chip.Toggle (ToggleChip)        | 스니펫 |
// | Avatar      | Avatar (Root/Fallback/Image)    | 스니펫 |
// | ListRow     | List.Item + Prefix/Content/Suffix | 런타임 |
// | Card        | 없음 → Box 조립                 | -      |
// | Progress    | 없음(원형만) → Box 2중첩 조립    | -      |
// | IconButton  | ActionButton layout="iconOnly"   | 런타임 |
//
// SEED 에 없어 Box 로 조립한 Card · 선형 Progress 는
// 전부 SEED 토큰(bg.* / stroke.* / radius / dimension)만 참조한다.
// IconButton 은 ActionButton 의 iconOnly 레이아웃을 사용한다.
// ═══════════════════════════════════════════════════════════════

// ───────────────────────────────────────────────────────────────
// Button → SEED ActionButton
//
// variant 매핑. SEED 는 variant 를 7종으로 고정하고 조합 규칙까지 정의한다.
//   primary → brandSolid    (화면당 1개 권장)
//   neutral → neutralSolid  (화면당 1개 권장)
//   outline → neutralOutline
//   subtle  → neutralWeak   ※ SEED 에 brandWeak 버튼이 없다.
//                             기존의 파란 틴트(#EAF2FE/#0054D1)는 SEED 체계에
//                             대응이 없어 뉴트럴 약 배경으로 흡수했다.
//   soft    → neutralWeak
//   ghost   → ghost
// ───────────────────────────────────────────────────────────────
const BUTTON_VARIANT_TO_SEED = {
  primary: 'brandSolid',
  neutral: 'neutralSolid',
  outline: 'neutralOutline',
  subtle: 'neutralWeak',
  soft: 'neutralWeak',
  ghost: 'ghost',
...
interface ButtonProps {
  variant?: keyof typeof BUTTON_VARIANT_TO_SEED;
...
function Button({
  ...
  // size 는 SEED 와 enum 이 동일하다 (xsmall · small · medium · large).
  return (
    <ActionButton
      type={type}
      variant={BUTTON_VARIANT_TO_SEED[variant] || 'brandSolid'}
      size={size}
      disabled={disabled}
      onClick={onClick}
...
// Badge → SEED Badge
//
// tone 은 SEED 의 **상태 축**(neutral·informative·positive·warning·critical·brand)에
// 매핑한다. 단 도감 4계열(자연·유적·문화·축제)은 상태가 아니라 **분류 축**이라
// SEED tone 으로 표현할 수 없다("자연 = positive" 는 의미가 깨진다).
// 그래서 계열 tone 3종은 SEED Badge 의 구조·크기·타이포는 그대로 쓰고
// 색만 도메인 토큰으로 덮는다.
//
// size: SEED 는 medium(20px/t1) · large(24px/t2) 2종뿐이다.
//   xsmall·small → medium,  medium → large 로 접었다. 실측 폰트가 각각
//   11px·12px 이라 SEED 두 단계와 그대로 일치한다.
// ───────────────────────────────────────────────────────────────
const BADGE_TONE_TO_SEED = {
  neutral: 'neutral',
  primary: 'brand',
  success: 'positive',
  warning: 'warning',
...
/** 4계열 확장 tone. SEED tone 축에 대응물이 없어 색만 주입한다. */
const BADGE_DOMAIN_TONE = {
  accent: { solid: 'var(--seed-color-palette-purple-700)', weak: 'var(--seed-color-palette-purple-200)', dark: 'var(--seed-color-palette-purple-900)' },   // 문화
  heritage: { solid: 'var(--seed-color-palette-yellow-700)', weak: 'var(--seed-color-palette-yellow-100)', dark: 'var(--seed-color-palette-yellow-900)' }, // 유적
  festival: { solid: 'var(--seed-color-palette-red-700)', weak: 'var(--seed-color-palette-red-100)', dark: 'var(--seed-color-palette-red-900)' }, // 축제
} as const;

const BADGE_SIZE_TO_SEED = { xsmall: 'medium', small: 'medium', medium: 'large' } as const;

interface BadgeProps {
  children: ReactNode;
  tone?: keyof typeof BADGE_TONE_TO_SEED | keyof typeof BADGE_DOMAIN_TONE;
  variant?: 'subtle' | 'weak' | 'solid' | 'outline';
  size?: keyof typeof BADGE_SIZE_TO_SEED;
  leading?: ReactNode;
  style?: CSSProperties;
}

function Badge({ children, tone = 'neutral', variant = 'subtle', size = 'small', leading, style }: BadgeProps) {
  // 우리 variant 이름 subtle 은 SEED 의 weak 다.
  const seedVariant: 'weak' | 'solid' | 'outline' = variant === 'subtle' ? 'weak' : variant;
  const seedSize = BADGE_SIZE_TO_SEED[size] || 'medium';
  const domain = tone in BADGE_DOMAIN_TONE
    ? BADGE_DOMAIN_TONE[tone as keyof typeof BADGE_DOMAIN_TONE]
    : undefined;

  let domainStyle;
  if (domain) {
    if (seedVariant === 'solid') {
      domainStyle = { background: domain.solid, color: 'var(--seed-color-palette-static-white)' };
    } else if (seedVariant === 'outline') {
      domainStyle = { boxShadow: `inset 0 0 0 1px ${domain.solid}`, color: domain.dark };
    } else {
      domainStyle = { background: domain.weak, color: domain.dark };
    }
  }
  ...
  return (
    <SeedBadge
      tone={domain
        ? 'neutral'
        : BADGE_TONE_TO_SEED[tone as keyof typeof BADGE_TONE_TO_SEED] || 'neutral'}
      variant={seedVariant}
      size={seedSize}
      style={{ gap: leading ? 4 : undefined, ...domainStyle, ...style }}
    >
      {leading}
      {children}
    </SeedBadge>
...
// ───────────────────────────────────────────────────────────────
// Chip → SEED Chip.Toggle
//
// SEED solid variant 의 checked 상태가 `bg.neutralInverted`(#171719 + 흰 글자)라
// 기존 활성 칩 디자인과 정확히 같다. 그대로 쓴다.
// 우리 `active`/`onClick` → SEED `checked`/`onCheckedChange`.
// ───────────────────────────────────────────────────────────────
interface ChipProps {
  children: ReactNode;
  active?: boolean;
...
function Chip({ children, active, onClick, leading, trailing, size = 'medium' }: ChipProps) {
  return (
    <ToggleChip
      size={size === 'small' ? 'small' : 'medium'}
      checked={!!active}
      onCheckedChange={() => onClick && onClick()}
    >
      {leading}
      {children}
      {trailing}
...
// Avatar → SEED Avatar
//
// SEED size 는 enum(20·24·36·42·48·56·64·80·96·108)이라 임의 px 을 가장 가까운
// 단계로 스냅한다. 배경색은 지역·유저 식별색이라 도메인 값이므로 root 에 주입한다
// (SEED Fallback 은 자체 배경이 없어 root 색이 그대로 보인다).
// ───────────────────────────────────────────────────────────────
const SEED_AVATAR_SIZES = ['20', '24', '36', '42', '48', '56', '64', '80', '96', '108'] as const;

function snapAvatarSize(px: number): (typeof SEED_AVATAR_SIZES)[number] {
  return SEED_AVATAR_SIZES.reduce(
    (best, s) => (Math.abs(Number(s) - px) < Math.abs(Number(best) - px) ? s : best),
    SEED_AVATAR_SIZES[0],
  );
}
...
function Avatar({ initial = '?', size = 36, color = 'var(--seed-color-bg-brand-solid)', ring }: AvatarProps) {
  const snapped = snapAvatarSize(size);
  return (
    <SeedAvatar
      size={snapped}
      fallback={initial}
      style={{
        ...
        fontSize: Math.round(Number(snapped) * 0.4),
        flexShrink: 0,
...
// ───────────────────────────────────────────────────────────────
// IconButton - SEED ActionButton(layout="iconOnly") 어댑터.
//
// SEED ActionButton 의 `layout="iconOnly"` 는 자식이 반드시 SEED `<Icon svg={} />`
// 여야 하고(아니면 dev 에서 throw), 우리 아이콘 헬퍼 `I` 는 Slot 이 넘기는
// className·ref 를 받지 않는 순수 함수 컴포넌트라 그 계약을 만족하지 못한다.
// 그래서 SEED 토큰만 참조하는 Box 조립으로 남긴다.
// ───────────────────────────────────────────────────────────────
// SEED ActionButton 의 layout="iconOnly" 를 쓴다. 크기·모양·상태는 SEED 레시피가 정한다.
// 자식은 반드시 SEED <Icon> 이어야 하며(IconRequired 가 dev 에서 검사한다),
// Icon 은 Radix Slot 으로 className/ref 를 자식에 병합하므로 I 가 그걸 받아 <svg> 로 넘긴다.
const ICON_BUTTON_SIZE_TO_SEED = { small: "small", medium: "medium", large: "large" } as const;
const ICON_BUTTON_VARIANT_TO_SEED = {
  outline: "neutralOutline",
  soft: "neutralWeak",
  ghost: "ghost",
  // 사진·지도 위에 얹히던 반투명 흰 버튼. SEED 에 미디어 오버레이 variant 가 없어
  // 불투명 연회색 알약인 neutralWeak 로 받는다(투명하면 어두운 아이콘이 묻힌다).
  inverse: "neutralWeak",
} as const;

const CARD_PADDING_TO_SEED = {
  0: 0,
  10: 'x2_5',
  12: 'x3',
  14: 'x3_5',
...
interface IconButtonProps {
  ...
  onClick?: MouseEventHandler<HTMLButtonElement>;
  size?: keyof typeof ICON_BUTTON_SIZE_TO_SEED;
  variant?: keyof typeof ICON_BUTTON_VARIANT_TO_SEED;
  ariaLabel: string;
  style?: CSSProperties;
}

function IconButton({ children, onClick, size = "medium", variant = "outline", ariaLabel, style }: IconButtonProps) {
  return (
    <ActionButton
      layout="iconOnly"
      size={ICON_BUTTON_SIZE_TO_SEED[size] || "medium"}
      variant={ICON_BUTTON_VARIANT_TO_SEED[variant] || "neutralOutline"}
      onClick={onClick}
      aria-label={ariaLabel}
      style={style}
    >
...
// ───────────────────────────────────────────────────────────────
// Card - SEED 갭. 범용 카드 컨테이너는 SEED rootage 스펙에도 없다.
// Box(bg · borderRadius · borderColor)로 조립한다.
// ───────────────────────────────────────────────────────────────
interface CardProps {
  children: ReactNode;
  padding?: keyof typeof CARD_PADDING_TO_SEED;
  radius?: keyof typeof CARD_RADIUS_TO_SEED;
  onClick?: MouseEventHandler<HTMLDivElement>;
  style?: CSSProperties;
}

function Card({ children, padding = 20, radius = 16, onClick, style }: CardProps) {
  ...
  return (
    <Box
      ...
      borderRadius={CARD_RADIUS_TO_SEED[radius]}
      padding={CARD_PADDING_TO_SEED[padding]}
      onClick={onClick}
...
// ───────────────────────────────────────────────────────────────
// ListRow → SEED List.Item
//
// SEED 는 좌우 패딩을 `spacingX.globalGutter` 로 잡는다. tokens.css 에서 그
// 토큰을 모두립 실측 20px 로 재정의해 뒀으므로 화면 좌우 정렬이 맞는다.
// `last` 구분선만 SEED 에 대응 개념이 없어 Box 로 감싸 처리한다.
// ───────────────────────────────────────────────────────────────
...
function ListRow({ leading, title, subtitle, trailing, onClick, last, dense }: ListRowProps) {
  return (
    <Box
      ...
      onClick={onClick}
    >
      <SeedList.Item style={dense ? { paddingBlock: 'var(--seed-dimension-x3)' } : undefined}>
        {leading && <SeedList.Prefix>{leading}</SeedList.Prefix>}
        <SeedList.Content>
          <SeedList.Title>{title}</SeedList.Title>
          {subtitle && <SeedList.Detail>{subtitle}</SeedList.Detail>}
        </SeedList.Content>
        {trailing && <SeedList.Suffix>{trailing}</SeedList.Suffix>}
      </SeedList.Item>
    </Box>
  );
}
...
// ───────────────────────────────────────────────────────────────
// Progress - SEED 갭. SEED 에는 ProgressCircle(원형)만 있고 선형이 없다.
// 도감 채움률 바에 필수라 Box 2중첩으로 조립한다.
// ───────────────────────────────────────────────────────────────
...
function Progress({ value, total, color = 'var(--seed-color-bg-brand-solid)', height = 6, showLabel = false }: ProgressProps) {
  const pct = total === 0 ? 0 : Math.max(0, Math.min(100, (value / total) * 100));
  const heightToken = height === 4 ? 'x1' : 'x1_5';
  return (
    <Box width="100%">
      <Box bg="bg.neutralWeak" borderRadius="full" height={heightToken} overflowX="hidden">
        <Box
          height={heightToken}
          borderRadius="full"
          style={{
            width: `${pct}%`,
            background: color,
            transition: 'width var(--seed-duration-d6) var(--seed-timing-function-easing)',
          }}
        />
      </Box>
      {showLabel && (
...
const PRESETS: Preset[] = [
  {
    id: 'p1', ownerId: 'u-hanradol', ownerName: '한라돌', ownerInitial: '한', ownerColor: 'var(--seed-color-fg-critical)',
  ...
  {
    id: 'p2', ownerId: 'u-dojangking', ownerName: '도장킹', ownerInitial: '도', ownerColor: 'var(--seed-color-fg-neutral)',
  ...
  {
    id: 'p3', ownerId: 'u-baromsa', ownerName: '바다러', ownerInitial: '바', ownerColor: 'var(--seed-color-bg-brand-solid)',
  ...
  {
    id: 'p4', ownerId: 'u-munhwa', ownerName: '문화사랑', ownerInitial: '문', ownerColor: 'var(--seed-color-palette-purple-700)',
  ...
  {
    id: 'p5', ownerId: 'u-oreum', ownerName: '오름지기', ownerInitial: '오', ownerColor: 'var(--seed-color-palette-carrot-700)',
...
const FEED: FeedItem[] = [
  { id: 'f1', user: '한라돌', initial: '한', region: '제주', placeId: 'seongsan', timeAgo: '방금',     titleEarned: '제주를 발견한 자', diversity: 47, color: 'var(--seed-color-fg-critical)' },
  { id: 'f2', user: '바다러', initial: '바', region: '강원', placeId: 'gyeongpo', timeAgo: '5분 전',   diversity: 51, color: 'var(--seed-color-bg-brand-solid)' },
  { id: 'f3', user: '도장킹', initial: '도', region: '경북', placeId: 'bulguksa', timeAgo: '12분 전',  titleEarned: '봉우리 수집가', diversity: 62, color: 'var(--seed-color-palette-red-900)' },
  { id: 'f4', user: '오름지기', initial: '오', region: '제주', placeId: 'darangshi', timeAgo: '34분 전', diversity: 38, color: 'var(--seed-color-palette-carrot-700)' },
  { id: 'f5', user: '북악산소년', initial: '북', region: '서울', placeId: 'bukhansan', timeAgo: '1시간 전', diversity: 28, color: 'var(--seed-color-palette-green-700)' },
  { id: 'f6', user: '문화사랑', initial: '문', region: '서울', placeId: 'changdeok', timeAgo: '2시간 전',  diversity: 44, color: 'var(--seed-color-palette-purple-700)' },
];

const LEADERS: Leader[] = [
  { rank: 1, name: '도장킹',     initial: '도', count: 287, diversity: 62, title: '전국 매트릭스', color: 'var(--seed-color-fg-neutral)' },
  { rank: 2, name: '한라돌',     initial: '한', count: 264, diversity: 58, title: '제주 탐험가',   color: 'var(--seed-color-fg-critical)' },
  { rank: 3, name: '바다러',     initial: '바', count: 251, diversity: 51, title: '해양 탐험가',   color: 'var(--seed-color-bg-brand-solid)' },
  { rank: 4, name: '문화사랑',   initial: '문', count: 233, diversity: 47, title: '문화재 수호자', color: 'var(--seed-color-palette-purple-700)' },
  { rank: 5, name: '오름지기',   initial: '오', count: 218, diversity: 38, title: '봉우리 수집가', color: 'var(--seed-color-palette-carrot-700)' },
  { rank: 6, name: '북악산소년', initial: '북', count: 197, diversity: 36, title: '봉우리 수집가', color: 'var(--seed-color-palette-green-700)' },
  { rank: 7, name: '나',         initial: '나', count: 142, diversity: 22, title: '제주를 발견한 자', me: true, color: 'var(--seed-color-bg-brand-solid)' },
  { rank: 8, name: '폭포러버',   initial: '폭', count: 128, diversity: 18, title: '계곡의 탐험가', color: 'var(--seed-color-fg-informative)' },
  { rank: 9, name: '미술관덕후', initial: '미', count: 115, diversity: 14, title: '큐레이터',       color: 'var(--seed-color-palette-red-700)' },
];

const COMPANIONS_MOUNTAIN: Companion[] = [
  { id: 'm1', name: '새벽기상러', initial: '새', region: '강원', target: '설악산',
    ...
    titles: ['봉우리 수집가', '현명한 하산자'], color: 'var(--seed-color-fg-warning)' },
...
interface DexProvinceScreenProps {
  regionId: string;
  onBack: () => void;
  onOpenSigun?: (regionId: string, sigunName: string | null) => void;
  onOpenPlace?: (placeId: string) => void;
  onOpenPreset?: (presetId: string) => void;
  onOpenSigunPicker?: (regionId: string) => void;
}

interface DexSigunPickerScreenProps {
  regionId: string;
  onBack: () => void;
  onOpenSigun?: (regionId: string, sigunName: string) => void;
}

interface DexRegionScreenProps {
  regionId: string;
  onBack: () => void;
  onOpenPlace: (placeId: string) => void;
  onCreatePreset?: (regionId: string) => void;
...
interface PlaceDetailScreenProps {
  placeId: string;
  onBack: () => void;
  onDiscover: (placeId: string) => void;
}
...
interface InfoRowProps extends LabelValueProps {
  last?: boolean;
}
...
interface DiscoverScreenProps {
  onDiscoverSuccess: (placeId: string) => void;
}
...
interface PresetCourseMapProps {
  preset: Preset;
  region?: Region;
  onOpenPlace?: (placeId: string) => void;
}
...
interface PresetCreateScreenProps {
  regionId: string;
  onBack: () => void;
...
interface UserDexCarouselProps {
  filled: RegionCollectionItem[];
  empty: RegionCollectionItem[];
  onOpenRegion?: (regionId: string) => void;
}

interface ForeignRegionMiniProps {
  region: Region;
  collected: number;
  onClick?: MouseEventHandler<HTMLDivElement>;
}
...
const getCategory = (id: string): Category => {
  ...
  if (!category) throw new Error(`알 수 없는 카테고리: ${id}`);
  return category;
};
const getGroup = (id: string): CategoryGroup => {
  const category = getCategory(id);
  const group = CATEGORY_GROUPS[category.group];
  if (!group) throw new Error(`알 수 없는 계열: ${category.group}`);
  return group;
};
const getRegion = (id: string): Region => {
  const region = REGIONS.find(r => r.id === id);
  if (!region) throw new Error(`알 수 없는 광역: ${id}`);
  return region;
};
const getPlace = (id: string): Place => {
  const place = PLACES.find(p => p.id === id);
  if (!place) throw new Error(`알 수 없는 자원: ${id}`);
  return place;
};
const placesInGroup = (regionId: string, groupId: string, secondaryIds: string[] | null = null): Place[] =>
  PLACES.filter(p => {
...
const SIGNATURE_CITIES: Record<string, SignatureCity[]> = {
  gangwon:   [
...
const signatureCities = (regionId: string): SignatureCity[] => SIGNATURE_CITIES[regionId] || [];

// 광역 = 챕터 메타포: 짧은 catchphrase + REGIONS 순서 기반 chapterNo
const REGION_CATCHPHRASES: Record<string, string> = {
  seoul:     '한강이 흐르는 도시',
  busan:     '바람의 항구',
  daegu:     '분지의 도시',
  incheon:   '관문과 섬',
...
const sigunsOf = (regionId: string): Sigun[] => {
  const names = SIGUN_NAMES[regionId as keyof typeof SIGUN_NAMES] || [];
...
const popularPlaces = (regionId: string, n = 8): Place[] =>
  PLACES
...
function congestionFor(placeId: string): CongestionDay[] {
  let h = 0;
...
const KOREA_PATHS: Record<string, string> = {
...
const KOREA_LABELS: Record<string, [number, number]> = {
  jeju: [251.1, 928.1],
...
export const KOREA_BBOX: Record<string, PathBox> = {
  "jeju": {
    "x": 192.87,
    "y": 896.17,
...
// 당근 SEED 토큰 위에 조립한다 (var(--seed-color-*) · var(--seed-dimension-*) 등)
...
function PhotoPlaceholder({ hue = 200, lift = 0, silhouette = false, style, rounded = 0 }: PhotoPlaceholderProps) {
  const L = Math.max(28, Math.min(74, 56 + lift * 40));
  const sat = silhouette ? 0 : 28;
  // 사진 자리표시자 - hue 값(장소별 데이터)에서 색을 절차적으로 만든다.
  // 고정 팔레트가 아니라 연속 색상환이라 SEED 토큰으로 표현할 수 없다.
  const c1 = silhouette ? 'var(--seed-color-palette-gray-1000)' : `hsl(${hue}, ${sat}%, ${L}%)`;
  const c2 = silhouette ? 'var(--seed-color-palette-gray-900)' : `hsl(${(hue + 16) % 360}, ${sat - 6}%, ${L + 6}%)`;
  return (
    <div style={{
      ...
      <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none"
        ...
        <path d="M0 78 L18 50 L30 60 L48 35 L66 55 L82 42 L100 70 L100 100 L0 100 Z"
          fill={silhouette ? 'var(--seed-color-palette-static-black)' : `hsl(${hue}, ${sat - 8}%, ${Math.max(18, L - 22)}%)`} />
      </svg>
      {/* dot grid texture */}
      <svg width="100%" height="100%" style={{ position: 'absolute', inset: 0, opacity: 0.08 }}>
        <defs>
          <pattern id={`dots-${hue}-${lift}`} x="0" y="0" width="12" height="12" patternUnits="userSpaceOnUse">
            <circle cx="2" cy="2" r="1" fill="var(--seed-color-palette-static-white)" />
          </pattern>
        </defs>
...
function Brand({ size = 22, color = 'var(--seed-color-fg-brand)' }: BrandProps) {
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x2)', whiteSpace: 'nowrap' }}>
      <svg width={size + 2} height={size + 2} viewBox="0 0 24 24" fill="none">
        <path d="M12 2C7.6 2 4 5.6 4 10c0 4.5 4 9.5 8 12 4-2.5 8-7.5 8-12 0-4.4-3.6-8-8-8z"
          fill={color} />
        <circle cx="12" cy="10" r="3" fill="var(--seed-color-palette-static-white)" />
      </svg>
      <span style={{
        fontFamily: 'inherit',
        fontWeight: 'var(--seed-font-weight-bold)', fontSize: size,
        color, lineHeight: 1, paddingTop: 'var(--seed-dimension-x0_5)',
      }}>모두립</span>
    </div>
  );
}
...
// ─────────────────────────────────────────────
// AppHeader (TopNavigation) - SEED 갭
//
...
function AppHeader({ title, subtitle, onBack, trailing, transparent = false, fg }: AppHeaderProps) {
  const titleColor = fg || 'var(--seed-color-fg-neutral)';
  const subColor = fg ? `${fg}b8` : 'var(--seed-color-fg-neutral-subtle)';
  return (
    <HStack
      alignItems="flex-start"
      gap="x2"
      style={{
        paddingTop: 'var(--seed-dimension-x14)',
    ...
    >
      {onBack && (
        <IconButton onClick={onBack} variant={fg ? 'inverse' : 'outline'} ariaLabel="뒤로"
          style={{ marginRight: 'var(--seed-dimension-x1)' }}>
          <I n="chevron-left" s={20} c={fg ? 'var(--seed-color-palette-static-white)' : 'var(--seed-color-fg-neutral)'} />
        </IconButton>
      )}
      <Box flexGrow={1} minWidth="0" style={{ paddingTop: onBack ? 4 : 0 }}>
        {subtitle && (
          <Box style={{
            fontSize: 'var(--seed-font-size-t3)',
            lineHeight: 'var(--seed-line-height-t3)',
            color: subColor, fontWeight: 'var(--seed-font-weight-bold)',
        ...
        {title && (
          <Box style={{
            ...
            color: titleColor, fontWeight: 'var(--seed-font-weight-bold)',
          }}>{title}</Box>
        )}
      </Box>
      {trailing && (
        <Box flexShrink={0} style={{ paddingTop: 'var(--seed-dimension-x1_5)' }}>{trailing}</Box>
      )}
    </HStack>
  );
}
...
// TabBar - 하단 탭 내비게이션
//
// SEED 는 하단 탭 내비게이션을 React 로 배포하지 않는다(BottomNavigation 없음).
// Box 로 조립하되 색·보더는 SEED 시맨틱 토큰을 쓴다.
// ─────────────────────────────────────────────
  ...
  return (
    <Box
      ...
      style={{
        ...
        paddingBottom: 'var(--seed-dimension-x7)', paddingTop: 'var(--seed-dimension-x2_5)',
      }}
    >
      {tabs.map(t => {
        ...
        return (
          <Box
            ...
            gap="x1"
            style={{
              background: 'none', border: 'none', padding: 'var(--seed-dimension-x1_5) var(--seed-dimension-x1)',
              cursor: 'pointer',
              color: isActive ? 'var(--seed-color-fg-brand)' : 'var(--seed-color-fg-neutral-subtle)',
              WebkitTapHighlightColor: 'transparent',
            }}
          >
            {/* 활성 표현은 Line ↔ Fill 스왑이다(SEED 자체가 쓰는 패턴). 선 굵기로는
                표현할 수 없다 - SEED 아이콘은 두께가 도형에 구워져 있다. */}
            <I n={t.icon} s={22} c="currentColor" fill={isActive ? 'currentColor' : 'none'} />
            <span style={{
              fontSize: 'var(--seed-font-size-t1)',
              fontWeight: isActive ? 'var(--seed-font-weight-bold)' : 'var(--seed-font-weight-medium)',
              lineHeight: 1,
            }}>{t.label}</span>
          </Box>
        );
...
// ─────────────────────────────────────────────
const __bboxCache: Record<string, PathBox> = {};
function pathBBox(d: string): PathBox {
...
export interface RegionSilhouetteProps {
  regionId: string;
  size?: string | number;
  fill?: string;
  stroke?: string;
...
function RegionSilhouette({ regionId, size = 56, fill, stroke = 'var(--seed-color-palette-static-white)', strokeWidth = 1, fillPct }: RegionSilhouetteProps) {
  ...
  if (fillPct !== undefined) {
    ...
    return (
      <svg viewBox={vb} width={size} height={size} style={{ display: 'block', overflow: 'visible' }}>
        ...
        <path d={d} fill="var(--seed-color-palette-gray-300)" stroke="var(--seed-color-palette-gray-500)"
...
// ─────────────────────────────────────────────
export interface KakaoNavIconProps {
  size?: number;
  ariaLabel?: string;
}

function KakaoNavIcon({ size = 36, ariaLabel = '카카오맵 길찾기' }: KakaoNavIconProps) {
  return (
    <button aria-label={ariaLabel} style={{
      width: size, height: size, padding: 0,
      borderRadius: '50%', border: 'none', cursor: 'pointer',
      // 카카오 브랜드 가이드가 지정한 고정색이다. 임의로 바꾸면 카카오 로그인
      // 심사에서 반려된다. SEED 토큰으로 치환하지 마라.
      background: '#FEE500',
...
const KOREA_RENDER_ORDER: string[] = [
...
const KOREA_LABEL_OVERRIDES: Record<string, [number, number]> = {
  seoul:   [311, 196],
  busan:   [666, 632],
  daegu:   [535, 504],
  incheon: [241, 230],
...
function KoreaMap({ selectedId, onSelect, height = 400 }: KoreaMapProps) {
  ...
  return (
    <svg viewBox={KOREA_VIEWBOX} width="100%" height={height} style={{ display: 'block', overflow: 'visible' }}>
      ...
      {KOREA_RENDER_ORDER.map(rid => {
        ...
        const filled = pct > 0;
        const fill = filled ? r.tone : 'var(--seed-color-bg-neutral-weak)';
        const alpha = filled ? (0.36 + pct * 0.56) : 1;
        return (
          <g key={rid} {...clickable(() => onSelect && onSelect(rid))} style={{ cursor: 'pointer' }}>
            <path d={d}
              fill={fill} fillOpacity={alpha}
              stroke="var(--seed-color-palette-gray-500)"
              strokeWidth={1.2}
      ...
      {selPath && selBBox && selRegion && selectedId && (() => {
        const cx = selBBox.x + selBBox.w / 2;
        const cy = selBBox.y + selBBox.h / 2;
        const fill = selFilled ? selRegion.tone : 'var(--seed-color-bg-neutral-weak)';
        const alpha = selFilled ? (0.6 + selPct * 0.4) : 1;
        return (
          <g {...clickable(() => onSelect && onSelect(selectedId))}
            style={{ cursor: 'pointer', transition: 'transform var(--seed-duration-d6) var(--seed-timing-function-easing)' }}
            transform={`translate(${cx} ${cy}) scale(1.14) translate(${-cx} ${-cy})`}>
            {/* outer glow */}
            <path d={selPath}
              fill="none" stroke="var(--seed-color-palette-static-black-alpha-600)"
              strokeWidth={14} strokeLinejoin="round"
              style={{ filter: 'blur(8px)' }}
            />
            <path d={selPath}
              fill={fill} fillOpacity={alpha}
              stroke="var(--seed-color-fg-neutral)" strokeWidth={2.8}
              strokeLinejoin="round"
            />
          </g>
        );
      ...
      {KOREA_RENDER_ORDER.map(rid => {
        ...
        const txt = filled ? 'var(--seed-color-palette-static-white)' : 'var(--seed-color-palette-gray-800)';
        const halo = filled ? 'var(--seed-color-palette-static-black-alpha-600)' : 'var(--seed-color-palette-static-white)';
        return (
          <g key={`label-${rid}`} style={{ pointerEvents: 'none' }}>
            <text x={x} y={y}
              textAnchor="middle"
              fontFamily="inherit" fontSize={fs} fontWeight="var(--seed-font-weight-bold)"
              fill={txt}
            ...
            {filled && !isSmall && (
              <text x={x} y={y + fs + 1}
                textAnchor="middle"
                fontFamily="inherit"
                fontSize={isSelected ? 14 : 12.5} fontWeight="var(--seed-font-weight-bold)"
                fill="var(--seed-color-palette-static-white-alpha-1000)"
                stroke="var(--seed-color-palette-static-black-alpha-600)" strokeWidth={2.8} strokeLinejoin="round"
                paintOrder="stroke fill">
                {Math.round(pct * 100)}%
              </text>
            )}
...
function SectionHeader({ title, subtitle, trailing, dense }: SectionHeaderProps) {
  return (
    <HStack
      alignItems="flex-end"
      ...
      gap="x2"
      style={{
        paddingTop: dense ? 'var(--seed-dimension-spacing-y-nav-to-title)' : 'var(--seed-dimension-x7)',
        paddingBottom: dense ? 'var(--seed-dimension-spacing-y-component-default)' : 'var(--seed-dimension-x3_5)',
        paddingLeft: 'var(--seed-dimension-spacing-x-global-gutter)',
        paddingRight: 'var(--seed-dimension-spacing-x-global-gutter)',
      }}
    >
      <Box minWidth="0">
        ...
        {subtitle && (
          <p style={{
            ...
            fontSize: 'var(--seed-font-size-t3)',
            lineHeight: 1.4, fontWeight: 'var(--seed-font-weight-medium)',
...
function OnboardingScreen({ onComplete }: OnboardingScreenProps) {
  ...
  const slides = [
    {
      eyebrow: '모두립',
      title: '가 본 곳이\n도감이 되는\n여행 앱',
      body: '전국 곳곳의 자연·유적·문화·축제를 직접 발견하고, 나만의 도감으로 모아보세요.',
      art: 'logo',
    },
    {
      eyebrow: '전국 도감',
      title: '17개 시·도를\n한 권에 모아요',
      body: '가 본 지역은 컬러로 채워지고, 아직 안 가 본 곳은 다음 여행지가 돼요. 오늘의 여행이 평생 남는 기록이 됩니다.',
      art: 'tiles',
    },
    {
      eyebrow: 'GPS 발견',
      title: '도착하면\n발견 버튼이\n켜져요',
      body: '여행지 근처에 들어가면 [발견] 버튼이 자동으로 활성화돼요. GPS 체크인부터 사진·후기까지, 원하는 만큼만 기록하세요.',
      art: 'gps',
    },
    {
      eyebrow: '칭호',
      title: '나만의 여행 취향을\n칭호로 남겨요',
      body: '"봉우리 수집가", "축제 마니아"처럼, 좋아하는 여행을 깊게 파고들수록 나만의 칭호가 쌓여요.',
      art: 'titles',
    },
    {
      eyebrow: '거의 다 왔어요',
      title: '뭐라고\n불러드릴까요?',
      body: '도감에 기록될 이름이에요. 언제든 바꿀 수 있어요.',
      art: 'name',
    },
  ];
  ...
  return (
    <div style={{
      ...
      background: onPrimary ? 'var(--seed-color-bg-brand-solid)' : 'var(--seed-color-palette-static-white)',
      color: onPrimary ? 'var(--seed-color-palette-static-white)' : 'var(--seed-color-fg-neutral)',
      display: 'flex', flexDirection: 'column',
      transition: 'background var(--seed-duration-d6), color var(--seed-duration-d6)',
      position: 'relative',
    }}>
      <div style={{ position: 'absolute', top: 60, right: 20, zIndex: 5 }}>
        {!isLast && (
          <button onClick={() => setStep(slides.length - 1)} style={{
            background: 'transparent', border: 'none', cursor: 'pointer', padding: 'var(--seed-dimension-x2)',
            fontSize: 'var(--seed-font-size-t4)', color: onPrimary ? 'var(--seed-color-palette-static-white-alpha-700)' : 'var(--seed-color-fg-neutral-subtle)',
            fontWeight: 'var(--seed-font-weight-bold)', fontFamily: 'inherit',
          }}>건너뛰기</button>
        )}
      </div>
      ...
      <div style={{
        ...
        {cur.art === 'name' && (
          <div style={{
            ...
            background: 'var(--seed-color-bg-brand-weak)', color: 'var(--seed-color-fg-brand)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
          }}>
            <I n="user" s={56} c="var(--seed-color-fg-brand)" w={1.6} />
          </div>
        )}
      </div>

      <div style={{ padding: '0 var(--seed-dimension-x7)', flex: 1 }}>
        <div style={{
          fontSize: 'var(--seed-font-size-t3)',
          lineHeight: 'var(--seed-line-height-t3)',
          fontWeight: 'var(--seed-font-weight-bold)',
          color: onPrimary ? 'var(--seed-color-palette-static-white-alpha-700)' : 'var(--seed-color-fg-neutral-subtle)',
          marginBottom: 'var(--seed-dimension-x4)',
        }}>{cur.eyebrow}</div>
        <div style={{
          fontSize: 'var(--seed-font-size-t10)',
          lineHeight: 'var(--seed-line-height-t10)',
          fontWeight: 'var(--seed-font-weight-bold)', whiteSpace: 'pre-line',
        }}>{cur.title}</div>
        <div style={{
          ...
          fontWeight: 'var(--seed-font-weight-medium)',
          color: onPrimary ? 'var(--seed-color-palette-static-white-alpha-800)' : 'var(--seed-color-fg-neutral-muted)',
        }}>{cur.body}</div>
        {cur.art === 'name' && (
      ...
      <div style={{
        display: 'flex', justifyContent: 'center', gap: 'var(--seed-dimension-x1_5)', marginBottom: 'var(--seed-dimension-x4_5)',
      }}>
        {slides.map((_, i) => (
          <div key={i} style={{
            width: i === step ? 22 : 6, height: 'var(--seed-dimension-x1_5)', borderRadius: 'var(--seed-radius-r1)',
            background: onPrimary ? 'var(--seed-color-palette-static-white)' : 'var(--seed-color-bg-neutral-inverted)',
            opacity: i === step ? 1 : 0.18,
            transition: 'all var(--seed-duration-d4)',
          }} />
        ))}
      </div>

      <div style={{
        padding: '0 var(--seed-dimension-x6) var(--seed-dimension-x10)',
        display: 'flex', gap: 'var(--seed-dimension-x2_5)',
      }}>
        {!isFirst && (
          <Button variant={onPrimary ? 'ghost' : 'soft'} size="large"
            onClick={() => setStep(step - 1)}
            style={onPrimary ? {
              background: 'var(--seed-color-palette-static-white-alpha-200)', color: 'var(--seed-color-palette-static-white)',
            } : undefined}>
            이전
          </Button>
        )}
        <div style={{ flex: 1 }}>
          <Button
            ...
            style={onPrimary ? { background: 'var(--seed-color-palette-static-white)', color: 'var(--seed-color-fg-brand)' } : undefined}
            trailing={<I n="arrow-right" s={18} c={onPrimary ? 'var(--seed-color-fg-brand)' : 'var(--seed-color-palette-static-white)'} />}
          >
            {isLast ? '도감 시작하기' : '계속'}
          </Button>
        </div>
  ...
  return (
    <div style={{ position: 'relative', width: 200, height: 200 }}>
      <svg viewBox="0 0 200 200" width="200" height="200">
        <circle cx="100" cy="100" r="100" fill="var(--seed-color-palette-static-white-alpha-100)" />
        <circle cx="100" cy="100" r="74" fill="var(--seed-color-palette-static-white-alpha-50)" />
        <path d="M100 36 C76 36 56 56 56 80 C56 110 86 138 100 158 C114 138 144 110 144 80 C144 56 124 36 100 36 Z"
          fill="var(--seed-color-palette-static-white)" />
        <circle cx="100" cy="80" r="18" fill="var(--seed-color-bg-brand-solid)" />
      </svg>
    </div>
  );
}
...
function OnbTilesArt() {
  ...
  type OnboardingTile =
    | { collected: true; group: string; icon: IconName }
    | { collected: false };
  const sample: OnboardingTile[] = [
  ...
  return (
    <div style={{
      display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--seed-dimension-x2)',
      width: 220,
    }}>
      {sample.map((t, i) => {
        if (t.collected) {
          ...
          return (
            <div key={i} style={{
              aspectRatio: '1', borderRadius: 'var(--seed-radius-r4)',
              background: g.color, color: 'var(--seed-color-palette-static-white)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              position: 'relative',
            }}>
              <I n={t.icon} s={28} c="var(--seed-color-palette-static-white)" w={2} />
              <span style={{
                position: 'absolute', top: 6, right: 6,
                width: 'var(--seed-dimension-x3_5)', height: 'var(--seed-dimension-x3_5)', borderRadius: '50%',
        ...
        return (
          <div key={i} style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 'var(--seed-font-size-t9)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-gray-600)',
            }}>?</div>
        );
      })}
    </div>
  ...
  return (
    <div style={{ width: 240, height: 240, position: 'relative' }}>
      <div style={{
        ...
        border: '2px dashed var(--seed-color-stroke-brand-solid)', opacity: 0.4,
        transform: 'scale(0.7)',
        animation: 'seed-exit 2s var(--seed-timing-function-exit) infinite',
        '--seed-exit-scale': 1.6, '--seed-exit-opacity': 0,
      } as SeedCSSProperties} />
      <div style={{
        position: 'absolute', inset: 60, borderRadius: '50%',
        border: '2px solid var(--seed-color-stroke-brand-solid)', opacity: 0.65,
        transform: 'scale(0.7)',
        animation: 'seed-exit 2s var(--seed-timing-function-exit) infinite 0.3s',
        '--seed-exit-scale': 1.6, '--seed-exit-opacity': 0,
      } as SeedCSSProperties} />
      <div style={{
        position: 'absolute', left: '50%', top: '50%',
        transform: 'translate(-50%, -50%)',
        width: 80, height: 80, borderRadius: 'var(--seed-radius-r5)',
        background: 'var(--seed-color-bg-brand-solid)', color: 'var(--seed-color-palette-static-white)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontWeight: 'var(--seed-font-weight-bold)', fontSize: 'var(--seed-font-size-t5)',
      }}>발견</div>
    </div>
  );
}
...
function OnbTitlesArt() {
  const sample: { tier: string; name: string; x: number; icon: IconName }[] = [
  ...
  return (
    <div style={{
      display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x2_5)', alignItems: 'center',
    }}>
      {sample.map((t, i) => {
        const tier = TITLE_TIERS[t.tier];
        return (
          <div key={i} style={{
            display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x2)',
            padding: 'var(--seed-dimension-x2_5) var(--seed-dimension-x3_5)', borderRadius: 'var(--seed-radius-full)',
            background: 'var(--seed-color-palette-static-white)',
            ...
            <span style={{
              width: 26, height: 26, borderRadius: 'var(--seed-radius-r2)',
              background: tier.color + '1a', color: tier.color,
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            }}><I n={t.icon} s={14} c={tier.color} /></span>
            <span style={{
              fontWeight: 'var(--seed-font-weight-bold)', fontSize: 'var(--seed-font-size-t4)', color: 'var(--seed-color-fg-neutral)',
              }}>{t.name}</span>
...
// Home Screen
// ─────────────────────────────────────────────
function HomeScreen({ user, onNavigate, onOpenRegion, onOpenPlace }: HomeScreenProps) {
  ...
  const busyPlace = getPlace(reco.busy.placeId);
  const altPlace = getPlace(reco.alt.placeId);
  const busyRegion = getRegion(reco.busy.region);
  const altRegion = getRegion(reco.alt.region);

  const recent = PLACES.filter((p): p is Place & { date: string } => p.collected && !!p.date)
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)' }}>
      {/* Top: brand bar */}
      <div style={{
        paddingTop: 'var(--seed-dimension-x14)', padding: 'var(--seed-dimension-x14) var(--seed-dimension-spacing-x-global-gutter) 0',
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      }}>
        <Brand size={20} color="var(--seed-color-fg-brand)" />
        <div style={{ display: 'flex', gap: 'var(--seed-dimension-x2)' }}>
          <IconButton variant="ghost" size="small" ariaLabel="검색">
            <I n="search" s={20} c="var(--seed-color-fg-neutral)" />
          </IconButton>
          <IconButton variant="ghost" size="small" ariaLabel="알림">
            <I n="bell" s={20} c="var(--seed-color-fg-neutral)" />
          </IconButton>
        </div>
      </div>

      {/* Greeting + Display number */}
      <div style={{ padding: 'var(--seed-dimension-x6) var(--seed-dimension-x5) var(--seed-dimension-x6)' }}>
        <div style={{
          fontSize: 'var(--seed-font-size-t5)', lineHeight: 'var(--seed-line-height-t5)',
          color: 'var(--seed-color-fg-neutral-muted)', fontWeight: 'var(--seed-font-weight-bold)',
      ...
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)' }}>
        <Card padding={0} radius={16} style={{
          ...
          background: 'var(--seed-color-bg-layer-basement)', cursor: 'pointer',
      ...
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)' }}>
        <Card padding={0} radius={20}>
          <div style={{ display: 'flex' }}>
            {/* busy */}
            <div {...clickable(() => onOpenPlace(busyPlace.id))} style={{
              flex: 1, padding: 'var(--seed-dimension-x4)', cursor: 'pointer',
              ...
              <div style={{
                ...
                fontSize: 'var(--seed-font-size-t5)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
                }}>{reco.busy.name}</div>
              <div style={{
                fontSize: 'var(--seed-font-size-t3)', color: 'var(--seed-color-fg-neutral-subtle)', fontWeight: 'var(--seed-font-weight-medium)', marginTop: 'var(--seed-dimension-x0_5)',
              }}>{busyRegion.name}</div>
              <div style={{ marginTop: 'var(--seed-dimension-x2_5)' }}>
                <Progress value={reco.busy.score} total={100} color="var(--seed-color-fg-critical)" height={5} />
              </div>
            </div>

            {/* arrow */}
            <div style={{
              display: 'flex', alignItems: 'center', padding: '0 var(--seed-dimension-x1_5)',
              color: 'var(--seed-color-fg-brand)',
            }}>
              <I n="arrow-right" s={20} c="var(--seed-color-fg-brand)" w={2.2} />
            </div>

            {/* alt */}
            <div {...clickable(() => onOpenPlace(altPlace.id))} style={{
              flex: 1, padding: 'var(--seed-dimension-x4)', cursor: 'pointer',
            }}>
              ...
              <div style={{ marginTop: 'var(--seed-dimension-x2_5)' }}>
                <Progress value={reco.alt.score} total={100} color="var(--seed-color-palette-green-700)" height={5} />
              </div>
            </div>
          </div>
          <div style={{
            padding: 'var(--seed-dimension-x3) var(--seed-dimension-x4)', borderTop: '1px solid var(--seed-color-stroke-neutral-subtle)',
            background: 'var(--seed-color-bg-layer-basement)',
            display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x2)',
            fontSize: 'var(--seed-font-size-t2)', color: 'var(--seed-color-fg-neutral-muted)', fontWeight: 'var(--seed-font-weight-medium)',
          }}>
            <I n="info" s={14} c="var(--seed-color-fg-neutral-subtle)" />
      ...
      <div style={{
        display: 'flex', gap: 'var(--seed-dimension-x3)', overflowX: 'auto', padding: '0 var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-x1)',
        scrollbarWidth: 'none', msOverflowStyle: 'none',
      }}>
        {recent.map(p => {
          const grp = getGroup(p.category);
          return (
            <div key={p.id} {...clickable(() => onOpenPlace(p.id))} style={{
              width: 156, flexShrink: 0, cursor: 'pointer',
            }}>
              <div style={{
                position: 'relative', borderRadius: 'var(--seed-radius-r4)', overflow: 'hidden',
                height: 112,
                border: '1px solid var(--seed-color-stroke-neutral-subtle)',
              }}>
                ...
                <div style={{ position: 'absolute', top: 8, right: 8 }}>
                  <Badge tone="success" variant="solid" size="xsmall"
                    leading={<I n="check" s={10} c="var(--seed-color-palette-static-white)" w={2.5} />}>
                    발견
                  </Badge>
                </div>
              </div>
              <div style={{
                marginTop: 'var(--seed-dimension-x2_5)', fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)',
                color: 'var(--seed-color-fg-neutral)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
              }}>{p.name}</div>
              <div style={{
                marginTop: 'var(--seed-dimension-x1)', fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)',
                color: 'var(--seed-color-fg-neutral-subtle)', display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1)',
              }}>
                <span style={{
                  display: 'inline-block', width: 'var(--seed-dimension-x1_5)', height: 'var(--seed-dimension-x1_5)', borderRadius: 'var(--seed-radius-r1)',
      ...
      <div style={{
        ...
        display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--seed-dimension-x2_5)',
      }}>
...
function UntouchedRegionsCarousel({ onOpenRegion }: RegionActionProps) {
  ...
  return (
    <>
      <SectionHeader title="아직 안 가본 지역" subtitle="다음 정복지를 골라보세요" trailing={
        <div style={{ display: 'flex', gap: 'var(--seed-dimension-x1)' }}>
          {Array.from({ length: pages }).map((_, i) => (
            <div key={i} style={{
              width: i === page ? 16 : 6, height: 'var(--seed-dimension-x1)', borderRadius: 'var(--seed-radius-r1)',
              background: i === page ? 'var(--seed-color-bg-neutral-inverted)' : 'var(--seed-color-palette-gray-500)',
              transition: 'all var(--seed-duration-d5)',
            }} />
          ))}
        </div>
      } />
      <div style={{
        display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--seed-dimension-x2_5)', padding: '0 var(--seed-dimension-spacing-x-global-gutter)',
      }} key={page}>
        {view.map((r, i) => (
          <div key={r.id + '-' + i} style={{
            animation: 'seed-enter 320ms var(--seed-timing-function-enter) both',
            '--seed-enter-opacity': 0, '--seed-enter-translate-y': '10px',
...
function StatColumn({ value, unit, label }: StatColumnProps) {
  return (
    <div style={{
      flex: 1, padding: 'var(--seed-dimension-x4) var(--seed-dimension-x2)', textAlign: 'center',
      display: 'flex', flexDirection: 'column', justifyContent: 'center',
    }}>
      <div style={{
        fontSize: 'var(--seed-font-size-t8)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
        lineHeight: 1,
        fontVariantNumeric: 'tabular-nums',
      }}>
        {value}
        <span style={{
          fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)', marginLeft: 'var(--seed-dimension-x0_5)',
        }}>{unit}</span>
      </div>
      <div style={{
        fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)',
        marginTop: 'var(--seed-dimension-x1_5)', }}>{label}</div>
    </div>
  );
}

function StatDivider() {
  return <div style={{ width: 1, background: 'var(--seed-color-stroke-neutral-subtle)' }} />;
}

function ActionCard({ icon, title, sub, onClick, primary }: ActionCardProps) {
  return (
    <button onClick={onClick} style={{
      background: primary ? 'var(--seed-color-bg-brand-solid)' : 'var(--seed-color-bg-neutral-weak)',
      color: primary ? 'var(--seed-color-palette-static-white)' : 'var(--seed-color-fg-neutral)',
      border: 'none', borderRadius: 'var(--seed-radius-r4)', padding: 'var(--seed-dimension-x4_5)', cursor: 'pointer',
      textAlign: 'left',
      display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x3)',
      WebkitTapHighlightColor: 'transparent',
      transition: 'background var(--seed-duration-d2)',
    }}>
      <span style={{
        width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x9)', borderRadius: 'var(--seed-radius-r3)',
        background: primary ? 'var(--seed-color-palette-static-white-alpha-300)' : 'var(--seed-color-palette-static-white)',
        display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
      }}><I n={icon} s={20} c={primary ? 'var(--seed-color-palette-static-white)' : 'var(--seed-color-fg-brand)'} w={2} /></span>
      <div>
        <div style={{ fontSize: 'var(--seed-font-size-t5)', fontWeight: 'var(--seed-font-weight-bold)' }}>{title}</div>
        <div style={{
          fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)', marginTop: 'var(--seed-dimension-x0_5)',
          color: primary ? 'var(--seed-color-palette-static-white-alpha-700)' : 'var(--seed-color-fg-neutral-subtle)',
        }}>{sub}</div>
      </div>
    </button>
  );
...
function RegionMiniCard({ region, onClick }: RegionMiniCardProps) {
  const pct = region.collected / region.total;
  const filled = pct > 0;
  return (
    <Card padding={14} radius={14} onClick={onClick}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x2_5)', marginBottom: 'var(--seed-dimension-x2_5)' }}>
        <span style={{
          width: 44, height: 44, borderRadius: 'var(--seed-radius-r3)',
          background: 'var(--seed-color-bg-layer-basement)',
          display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
          flexShrink: 0, padding: 'var(--seed-dimension-x1)',
        }}>
          <RegionSilhouette regionId={region.id} size={36}
            fill={region.tone}
            fillPct={pct}
            stroke="var(--seed-color-palette-gray-600)" strokeWidth={1} />
        </span>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{
            ...
            marginTop: 'var(--seed-dimension-x0_5)', fontVariantNumeric: 'tabular-nums',
...
// ─────────────────────────────────────────────
function DexNationScreen({ onOpenRegion }: RegionActionProps) {
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)' }}>
      <AppHeader subtitle="전국 도감" title="대한민국" />

      {/* 지도 ↔ 카드 전환.
          SEED SegmentedControl 은 아이템 min-width 86px + padding-inline 24px 라
          2단만 해도 180px 이 필요하다. 402px 프레임의 헤더 trailing 슬롯에는 들어가지
          않으므로, SEED 가 의도한 대로 헤더 아래 전폭 행으로 내렸다. */}
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-spacing-y-component-default)' }}>
        <ViewToggle
          options={[
            { id: 'map',   label: '지도' },
            { id: 'cards', label: '카드' },
      ...
      {/* Top stat strip */}
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-x4)' }}>
        <Card padding={0} radius={16} style={{
          display: 'flex', alignItems: 'stretch',
          border: '1px solid var(--seed-color-stroke-neutral-subtle)',
          background: 'var(--seed-color-bg-layer-basement)',
...
// 지도 ↔ 카드 전환 - SEED SegmentedControl 로 교체.
// 자체 구현이던 pill 토글과 SEED 의 세그먼티드 컨트롤은 역할이 정확히 같다.
// SEED 는 `aria-label` 또는 `aria-labelledby` 가 없으면 경고 후 실패하므로 반드시 준다.
function ViewToggle({ options, value, onChange, ariaLabel = '보기 방식' }: ViewToggleProps) {
  return (
    <SegmentedControl
      aria-label={ariaLabel}
      value={value}
    ...
    >
      {options.map(o => (
        <SegmentedControlItem key={o.id} value={o.id}>{o.label}</SegmentedControlItem>
      ))}
    </SegmentedControl>
  );
}

function MapView({ selectedId, onSelect, onOpenRegion, sel, selPct }: MapViewProps) {
  return (
    <div>
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)' }}>
        <Card padding={16} radius={20}>
          <div style={{
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            marginBottom: 'var(--seed-dimension-x2_5)',
          }}>
            <div style={{
              fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
              }}>다녀온 지역</div>
            <div style={{
              ...
              fontSize: 'var(--seed-font-size-t1)', color: 'var(--seed-color-fg-neutral-subtle)', fontWeight: 'var(--seed-font-weight-bold)',
              }}>
              {[0.32, 0.6, 0.88].map(a => (
                <div key={a} style={{
                  width: 'var(--seed-dimension-x2_5)', height: 'var(--seed-dimension-x2_5)', borderRadius: 'var(--seed-radius-r1)',
                  background: 'var(--seed-color-bg-brand-solid)', opacity: a,
                }} />
              ))}
              <span style={{ marginLeft: 'var(--seed-dimension-x1)' }}>채움률</span>
            </div>
          </div>
      ...
      {sel && (
        <div style={{ padding: 'var(--seed-dimension-x3_5) var(--seed-dimension-x5) 0' }}>
          <Card padding={0} radius={16} onClick={() => onOpenRegion(sel.id)}>
            <div style={{
              padding: 'var(--seed-dimension-x4)', display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3_5)',
            }}>
              <span style={{
                width: 'var(--seed-dimension-x14)', height: 'var(--seed-dimension-x14)', borderRadius: 'var(--seed-radius-r4)',
                background: 'var(--seed-color-bg-layer-basement)',
                display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                flexShrink: 0, padding: 'var(--seed-dimension-x1_5)',
              }}>
                <RegionSilhouette regionId={sel.id} size={44}
                  fill={selPct > 0 ? sel.tone : 'var(--seed-color-palette-gray-500)'} stroke="transparent" strokeWidth={0} />
              </span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  fontSize: 'var(--seed-font-size-t6)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
                  lineHeight: 1.3,
                }}>{sel.full
                  .replace('특별시', '').replace('광역시', '')
                  .replace('특별자치도', '').replace('특별자치시', '')}</div>
                <div style={{
                  fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-subtle)',
                  marginTop: 'var(--seed-dimension-x0_5)', fontVariantNumeric: 'tabular-nums',
                }}>
                  {sel.collected > 0
              ...
              <I n="chevron-right" s={16} c="var(--seed-color-fg-neutral-subtle)" />
            </div>
          </Card>
        </div>
      )}
      ...
      {/* Hint */}
      <div style={{
        margin: 'var(--seed-dimension-x3_5) var(--seed-dimension-x5) 0',
        padding: 'var(--seed-dimension-x3) var(--seed-dimension-x3_5)',
        background: 'var(--seed-color-palette-blue-100)',
        border: '1px solid var(--seed-color-stroke-brand-weak)',
        borderRadius: 'var(--seed-radius-r3)',
        display: 'flex', alignItems: 'flex-start', gap: 'var(--seed-dimension-x2_5)',
      }}>
        <I n="info" s={16} c="var(--seed-color-fg-brand)" style={{ flexShrink: 0, marginTop: 'var(--seed-dimension-x0_5)' }} />
        <div style={{
          fontSize: 'var(--seed-font-size-t2)', color: 'var(--seed-color-fg-neutral-muted)', fontWeight: 'var(--seed-font-weight-medium)',
          lineHeight: 1.55, }}>지역을 탭하면 색이 진해진 만큼 다녀왔다는 뜻이에요. 카드를 한 번 더 누르면 도감이 열립니다.</div>
      </div>
    </div>
  );
}

function RegionGrid({ onOpenRegion }: RegionActionProps) {
  return (
    <div style={{
      padding: '0 var(--seed-dimension-spacing-x-global-gutter)',
      display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--seed-dimension-x2_5)',
...
// ─────────────────────────────────────────────
function DexProvinceScreen({ regionId, onBack, onOpenSigun, onOpenPlace, onOpenPreset, onOpenSigunPicker }: DexProvinceScreenProps) {
  const region = getRegion(regionId);
  ...
  const sigunFor = (name: string): Sigun | undefined => siguns.find(s => s.name === name);
  const cityChips: { name: string; sigun: Sigun }[] = (sigCities.length > 0
    ? sigCities.flatMap(c => {
        const sigun = sigunFor(c.name);
        return sigun ? [{ name: c.name, sigun }] : [];
      })
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)', position: 'relative' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          padding: 'var(--seed-dimension-x13) var(--seed-dimension-x4) 0',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        }}>
          <IconButton variant="inverse" onClick={onBack} ariaLabel="뒤로">
            <I n="chevron-left" s={20} c="var(--seed-color-palette-static-white)" />
          </IconButton>
          <div style={{
            padding: 'var(--seed-dimension-x1_5) var(--seed-dimension-x3)', borderRadius: 'var(--seed-radius-full)',
            background: 'var(--seed-color-palette-static-white-alpha-300)',
            fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-static-white)',
            fontVariantNumeric: 'tabular-nums',
          }}>{found}곳 발견</div>
        </div>
        ...
        <div style={{
          ...
          padding: '0 var(--seed-dimension-x6)',
        }}>
          <div style={{
            fontSize: 'var(--seed-font-size-t10)', fontWeight: 'var(--seed-font-weight-bold)',
            color: 'var(--seed-color-palette-static-white)', lineHeight: 1,
          }}>{region.name}</div>
          {catchphrase && (
            <div style={{
              fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-medium)', marginTop: 'var(--seed-dimension-x3_5)',
              color: 'var(--seed-color-palette-static-white-alpha-800)', textAlign: 'center',
            }}>{catchphrase}</div>
          )}
        </div>
        ...
        <div style={{
          ...
          <div style={{
            height: 3, background: 'var(--seed-color-palette-static-white-alpha-300)',
            borderRadius: 'var(--seed-radius-full)', overflow: 'hidden',
          }}>
            <div style={{
              width: `${visitedRatio * 100}%`,
              height: '100%', background: 'var(--seed-color-palette-static-white)',
            }} />
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        padding: 'var(--seed-dimension-x2_5) var(--seed-dimension-x5) var(--seed-dimension-x7)',
      }}>
        {/* drag handle */}
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 'var(--seed-dimension-x3_5)' }}>
          <span style={{
            width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x1)', borderRadius: 'var(--seed-radius-r1)',
        ...
        <div>
          <div style={{
            fontSize: 'var(--seed-font-size-t7)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
            }}>어디로 갈까요?</div>
          <div style={{
            fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-subtle)',
            marginTop: 'var(--seed-dimension-x1)',
        ...
        <div style={{ marginTop: 'var(--seed-dimension-x6)' }}>
          <div style={{
            ...
            marginBottom: 'var(--seed-dimension-x2_5)', marginRight: -20, paddingRight: 'var(--seed-dimension-x5)',
          }}>
            <div style={{
              fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
              }}>인기 자원</div>
            <button onClick={() => onOpenSigun?.(regionId, null)} style={{
              display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x0_5)',
              padding: 0, background: 'transparent', border: 'none',
              cursor: 'pointer', WebkitTapHighlightColor: 'transparent',
              fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)',
            }}>
              모두 보기
              <I n="chevron-right" s={12} c="var(--seed-color-fg-neutral-subtle)" />
            </button>
          </div>
          <div style={{
            display: 'flex', gap: 'var(--seed-dimension-x3)', overflowX: 'auto',
            marginRight: -20, paddingRight: 'var(--seed-dimension-x5)', paddingBottom: 'var(--seed-dimension-x1)',
            scrollbarWidth: 'none', msOverflowStyle: 'none',
          }}>
            {popular.map(p => (
        ...
        <div style={{ marginTop: 'var(--seed-dimension-x7)' }}>
          <div style={{
            ...
            {presets.length > 3 && (
              <button onClick={() => setShowAllPresets(v => !v)} style={{
                ...
                <I n={showAllPresets ? 'chevron-down' : 'chevron-right'} s={12} c="var(--seed-color-fg-neutral-subtle)"
          ...
          {presets.length === 0 ? (
            <div style={{
              padding: 'var(--seed-dimension-x4_5)', textAlign: 'center',
              background: 'var(--seed-color-bg-layer-basement)', borderRadius: 'var(--seed-radius-r3)',
        ...
        <div style={{ marginTop: 'var(--seed-dimension-x7)' }}>
          ...
          {cityChips.length > 0 && (
            <div style={{
              display: 'flex', flexWrap: 'wrap', gap: 'var(--seed-dimension-x1_5)', marginBottom: 'var(--seed-dimension-x2_5)',
            }}>
          ...
          <button onClick={() => onOpenSigunPicker?.(regionId)} style={{
            ...
            gap: 'var(--seed-dimension-x1_5)', padding: 'var(--seed-dimension-x3) var(--seed-dimension-x3_5)', borderRadius: 'var(--seed-radius-r3)',
            background: 'transparent',
            border: '1px dashed var(--seed-color-stroke-neutral-muted)',
            cursor: 'pointer', WebkitTapHighlightColor: 'transparent',
            fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)',
          }}>
...
// ─────────────────────────────────────────────
function DexSigunPickerScreen({ regionId, onBack, onOpenSigun }: DexSigunPickerScreenProps) {
  const region = getRegion(regionId);
  ...
  const byName = (a: Sigun, b: Sigun): number => a.name.localeCompare(b.name, 'ko');
  ...
  const renderRow = (s: Sigun): React.ReactElement => {
    const isV = s.collected > 0;
    return (
      <button key={s.id} onClick={() => onOpenSigun?.(regionId, s.name)} style={{
        width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        padding: 'var(--seed-dimension-x3) var(--seed-dimension-x5)', background: 'transparent', border: 'none',
        cursor: 'pointer', WebkitTapHighlightColor: 'transparent', textAlign: 'left',
      }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x2_5)' }}>
          <span style={{
            width: 'var(--seed-dimension-x1_5)', height: 'var(--seed-dimension-x1_5)', borderRadius: 'var(--seed-radius-full)',
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)', minHeight: '100%' }}>
      {/* Header */}
      <div style={{ padding: 'var(--seed-dimension-x13) var(--seed-dimension-x5) var(--seed-dimension-x3_5)', borderBottom: '0.5px solid var(--seed-color-stroke-neutral-subtle)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x2_5)', marginBottom: 'var(--seed-dimension-x3_5)' }}>
          <IconButton onClick={onBack} ariaLabel="뒤로">
            <I n="chevron-left" s={20} c="var(--seed-color-fg-neutral-muted)" />
          </IconButton>
          <div style={{ flex: 1 }}>
            <div style={{
              fontSize: 'var(--seed-font-size-t5)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
        ...
        <TextInput
          ...
          suffix={query.length > 0 ? (
            <button onClick={() => setQuery('')} aria-label="검색어 지우기" style={{
              ...
              background: 'var(--seed-color-stroke-neutral-muted)', border: 'none', padding: 0,
              cursor: 'pointer',
            }}>
      ...
      {matches.length === 0 ? (
        <div style={{
          padding: 'var(--seed-dimension-x16) var(--seed-dimension-x5)', textAlign: 'center',
        ...
        <div>
          {visited.length > 0 && (
            <>
              <div style={{
                padding: 'var(--seed-dimension-x3_5) var(--seed-dimension-x5) var(--seed-dimension-x1_5)',
                fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)',
...
// ─────────────────────────────────────────────
function DexRegionScreen({ regionId, onBack, onOpenPlace, onCreatePreset, sigunName }: DexRegionScreenProps) {
  const region = getRegion(regionId);
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--seed-dimension-x2)', justifyContent: 'space-between' }}>
          ...
          {regionFound >= 2 && (
            <Button variant="neutral" size="small" onClick={() => onCreatePreset?.(regionId)}
              leading={<I n="plus" s={14} c="var(--seed-color-palette-static-white)" w={2.5} />}
              style={{ background: 'var(--seed-color-palette-static-white-alpha-200)', color: 'var(--seed-color-palette-static-white)', border: 'none' }}>
              프리셋 만들기
            </Button>
          )}
        </div>
        <div style={{ marginTop: 'var(--seed-dimension-x4)', display: 'flex', alignItems: 'flex-end', gap: 'var(--seed-dimension-x3)' }}>
          <span style={{
            width: 'var(--seed-dimension-x16)', height: 'var(--seed-dimension-x16)', borderRadius: 'var(--seed-radius-r4)',
            background: 'var(--seed-color-palette-static-white-alpha-200)',
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            flexShrink: 0, padding: 'var(--seed-dimension-x1_5)',
          }}>
            <RegionSilhouette regionId={region.id} size={52}
              fill="var(--seed-color-palette-static-white)" stroke="transparent" strokeWidth={0} />
          </span>
          <div style={{ flex: 1 }}>
            <div style={{
              fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-static-white-alpha-700)',
              }}>{sigunName ? `${region.name} · 시군 도감` : '지역 도감'}</div>
            <div style={{
              fontSize: 'var(--seed-font-size-t9)',
              lineHeight: 'var(--seed-line-height-t9)',
              fontWeight: 'var(--seed-font-weight-bold)', marginTop: 'var(--seed-dimension-x0_5)',
            }}>{sigunName || region.full
              .replace('특별시', '').replace('광역시', '')
              .replace('특별자치도', '').replace('특별자치시', '')}</div>
            <div style={{
              fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-bold)', marginTop: 'var(--seed-dimension-x1)',
              color: 'var(--seed-color-palette-static-white-alpha-800)', fontVariantNumeric: 'tabular-nums',
            }}>{region.collected} / {region.total}곳 발견</div>
          </div>
        </div>
        <div style={{ marginTop: 'var(--seed-dimension-x3_5)' }}>
          <Progress value={region.collected} total={region.total} color="var(--seed-color-palette-static-white)" height={6} />
        </div>
      </div>
      ...
      <div style={{ padding: 'var(--seed-dimension-x5) var(--seed-dimension-spacing-x-global-gutter) 0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--seed-dimension-x2)' }}>
          {groups.map(g => {
            const stats = (() => {
            ...
            return (
              <button key={g.id}
                ...
                style={{
                  padding: 'var(--seed-dimension-x3) var(--seed-dimension-x1_5)', borderRadius: 'var(--seed-radius-r3)',
                  background: isActive ? g.color : 'var(--seed-color-bg-neutral-weak)',
                  border: 'none',
                  color: isActive ? 'var(--seed-color-palette-static-white)' : g.dark,
                  cursor: 'pointer', textAlign: 'center',
                  display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--seed-dimension-x1)',
                  transition: 'all var(--seed-duration-d2)',
                  WebkitTapHighlightColor: 'transparent',
                }}>
                <I n={g.icon} s={18} c={isActive ? 'var(--seed-color-palette-static-white)' : g.color} w={2} />
                <div style={{
                  fontWeight: 'var(--seed-font-weight-bold)', fontSize: 'var(--seed-font-size-t3)', }}>{g.label}</div>
                <div style={{
      ...
      <div style={{ padding: 'var(--seed-dimension-x5) var(--seed-dimension-spacing-x-global-gutter) 0' }}>
        ...
        <div style={{
          display: 'flex', gap: 'var(--seed-dimension-x1_5)', overflowX: 'auto', paddingBottom: 'var(--seed-dimension-x1)',
          scrollbarWidth: 'none', msOverflowStyle: 'none',
        }}>
      ...
      {/* Filter row */}
      <div style={{
        padding: 'var(--seed-dimension-x4) var(--seed-dimension-x5) var(--seed-dimension-x3)',
        ...
        <div style={{ display: 'flex', gap: 'var(--seed-dimension-x1_5)' }}>
          {[
            ...
            <button key={f.id} onClick={() => setFilter(f.id)} style={{
              padding: 'var(--seed-dimension-x1_5) var(--seed-dimension-x3)', borderRadius: 'var(--seed-radius-full)',
              background: filter === f.id ? 'var(--seed-color-bg-neutral-inverted)' : 'transparent',
              color: filter === f.id ? 'var(--seed-color-fg-neutral-inverted)' : 'var(--seed-color-fg-neutral-muted)',
              border: filter === f.id ? '1px solid var(--seed-color-bg-neutral-inverted)' : '1px solid var(--seed-color-stroke-neutral-muted)',
              cursor: 'pointer',
              fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', fontVariantNumeric: 'tabular-nums',
            }}>{f.label}</button>
          ))}
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div style={{
          margin: '0 var(--seed-dimension-x5) var(--seed-dimension-x5)',
          padding: 'var(--seed-dimension-x8) var(--seed-dimension-x5)', textAlign: 'center',
          background: 'var(--seed-color-bg-layer-basement)', borderRadius: 'var(--seed-radius-r4)',
          border: '1px dashed var(--seed-color-stroke-neutral-muted)',
          fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-muted)',
        }}>해당 조건의 자원이 없어요.</div>
      ) : (
        <div style={{
          padding: '0 var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-x5)',
          display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--seed-dimension-x3)',
        }}>
          {filtered.map(p => (
...
function PlaceTile({ place, onClick }: PlaceTileProps) {
  ...
  return (
    <Card
      ...
      style={{
        border: place.collected ? `1px solid ${grp.color}` : '1px solid var(--seed-color-stroke-neutral-subtle)',
      }}
    >
      <div style={{
        ...
        <div style={{
          ...
          transition: 'filter var(--seed-duration-d5)',
        }}>
        ...
        <div style={{
          ...
          padding: 'var(--seed-dimension-x0_5) var(--seed-dimension-x2)', borderRadius: 'var(--seed-radius-r1)',
          background: 'var(--seed-color-palette-static-black-alpha-800)', color: 'var(--seed-color-palette-static-white-alpha-900)',
          fontFamily: 'ui-monospace, monospace', fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)',
          }}>#{String(idx).padStart(3, '0')}</div>

        {userPhoto && place.collected && (
          <div style={{ position: 'absolute', bottom: 8, left: 8 }}>
            <Badge tone="primary" variant="solid" size="xsmall"
              leading={<I n="image" s={9} c="var(--seed-color-palette-static-white)" w={2.2} />}>
              내 사진
            </Badge>
          </div>
        )}
      </div>
      <div style={{ padding: 'var(--seed-dimension-x2_5) var(--seed-dimension-x3) var(--seed-dimension-x3)' }}>
        <div style={{
          fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)',
          color: place.collected ? 'var(--seed-color-fg-neutral)' : 'var(--seed-color-fg-neutral-subtle)',
          overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
        }}>{place.name}</div>
        <div style={{
          display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1)', marginTop: 'var(--seed-dimension-x1)',
          fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: grp.color, }}>
...
// Place Detail Screen
// ─────────────────────────────────────────────
function PlaceDetailScreen({ placeId, onBack, onDiscover }: PlaceDetailScreenProps) {
  ...
  const nearby: { iconName: IconName; name: string; dist: string; type: string }[] = [
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)' }}>
      <div style={{ position: 'relative', height: 320 }}>
        <div {...(place.collected ? clickable(() => setCardOpen(true)) : {})}
          style={{
            width: '100%', height: '100%',
        ...
        <div style={{
          ...
          padding: 'var(--seed-dimension-x14) var(--seed-dimension-x4) 0',
          display: 'flex', justifyContent: 'space-between',
        }}>
          <IconButton onClick={onBack} variant="inverse" ariaLabel="뒤로"
            style={{ background: 'var(--seed-color-palette-static-white-alpha-900)', border: 'none' }}>
            <I n="chevron-left" s={20} c="var(--seed-color-fg-neutral)" />
          </IconButton>
          <IconButton variant="inverse" ariaLabel="저장"
            style={{ background: 'var(--seed-color-palette-static-white-alpha-900)', border: 'none' }}>
            <I n="heart" s={20} c="var(--seed-color-fg-neutral)" />
          </IconButton>
        </div>
        ...
        {place.collected && (
          <div style={{
            ...
            display: 'flex', gap: 'var(--seed-dimension-x1_5)', alignItems: 'center',
          }}>
            {userPhoto && (
              <Badge tone="primary" variant="solid" size="small"
                leading={<I n="image" s={11} c="var(--seed-color-palette-static-white)" w={2.2} />}>
                내 사진
              </Badge>
            )}
            <Badge tone="success" variant="solid" size="medium"
              leading={<I n="check" s={14} c="var(--seed-color-palette-static-white)" w={2.5} />}>
              발견 완료
            </Badge>
          </div>
        )}
        ...
        <div style={{
          ...
          padding: 'var(--seed-dimension-x1) var(--seed-dimension-x2_5)', borderRadius: 'var(--seed-radius-r2)',
          background: 'var(--seed-color-palette-static-black-alpha-800)', color: 'var(--seed-color-palette-static-white-alpha-900)',
          fontFamily: 'ui-monospace, monospace', fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)',
      ...
      <div style={{
        background: 'var(--seed-color-palette-static-white)', marginTop: -24,
        borderTopLeftRadius: 24, borderTopRightRadius: 24,
        position: 'relative', zIndex: 2, paddingTop: 'var(--seed-dimension-x6)',
      }}>
        {/* Header */}
        <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)' }}>
          <div style={{ display: 'flex', gap: 'var(--seed-dimension-x1_5)', flexWrap: 'wrap', marginBottom: 'var(--seed-dimension-x3)' }}>
            <Badge tone={group.id === 'culture' ? 'accent' : group.id === 'festival' ? 'festival' : group.id === 'heritage' ? 'heritage' : 'success'}
              variant="solid" size="small"
              leading={<I n={group.icon} s={12} c="var(--seed-color-palette-static-white)" w={2} />}>
              {group.label}
            </Badge>
          ...
          <div style={{
            ...
            <div style={{ flexShrink: 0, paddingTop: 'var(--seed-dimension-x1_5)' }}>
        ...
        {place.collected && (
          <div style={{ padding: 'var(--seed-dimension-x4) var(--seed-dimension-spacing-x-global-gutter) 0' }}>
            ...
            <Card padding={14} radius={14} style={{
              display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3)',
              background: userPhoto ? 'var(--seed-color-bg-brand-weak)' : 'var(--seed-color-bg-layer-basement)',
              border: userPhoto ? '1px solid var(--seed-color-stroke-brand-solid)' : '1px solid var(--seed-color-stroke-neutral-subtle)',
            }}>
              <span style={{
                width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x9)', borderRadius: 'var(--seed-radius-r3)',
                background: userPhoto ? 'var(--seed-color-bg-brand-solid)' : 'var(--seed-color-palette-static-white)',
                color: userPhoto ? 'var(--seed-color-palette-static-white)' : 'var(--seed-color-fg-brand)',
                display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                border: userPhoto ? 'none' : '1px solid var(--seed-color-stroke-neutral-subtle)',
              }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{
                  fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)',
                  color: 'var(--seed-color-fg-neutral)', }}>{userPhoto ? '내 사진이 도감에 추가됐어요' : '내 사진으로 도감 꾸미기'}</div>
                <div style={{
        ...
        <div style={{ padding: 'var(--seed-dimension-x5) var(--seed-dimension-spacing-x-global-gutter) 0' }}>
          <Card padding={16} radius={14}>
            <div style={{
              fontSize: 'var(--seed-font-size-t4)',
              lineHeight: 'var(--seed-line-height-t5)',
              fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-muted)',
            }}>
              {place.collected
            ...
            <div style={{
              display: 'flex', gap: 0, marginTop: 'var(--seed-dimension-x4)',
              paddingTop: 'var(--seed-dimension-x4)', borderTop: '1px solid var(--seed-color-stroke-neutral-subtle)',
            }}>
        ...
        <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)' }}>
          <Card padding={16} radius={16}>
            ...
            <div style={{
              display: 'flex', gap: 'var(--seed-dimension-x4)', marginTop: 'var(--seed-dimension-x3_5)', paddingTop: 'var(--seed-dimension-x3)',
              borderTop: '1px solid var(--seed-color-stroke-neutral-subtle)',
              justifyContent: 'center',
            }}>
              <LegendChip color="var(--seed-color-palette-green-400)" label="한산" />
              <LegendChip color="var(--seed-color-palette-yellow-300)" label="보통" />
              <LegendChip color="var(--seed-color-palette-red-400)" label="혼잡" />
            </div>
          </Card>
        </div>
        ...
        <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)' }}>
          ...
          <div style={{ marginTop: 'var(--seed-dimension-x2_5)', display: 'flex', gap: 'var(--seed-dimension-x2)' }}>
            <Button variant="outline" size="small"
              leading={<I n="external" s={14} c="var(--seed-color-fg-neutral)" />}>
              공식 페이지
            </Button>
          </div>
        </div>
              ...
              <ListRow key={i}
                leading={
                  <span style={{
                    ...
                    background: 'var(--seed-color-bg-neutral-weak)',
                    display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                  }}><I n={n.iconName} s={16} c="var(--seed-color-fg-neutral-muted)" /></span>
                }
                title={n.name}
                subtitle={`${n.type} · ${n.dist}`}
                trailing={<I n="chevron-right" s={16} c="var(--seed-color-fg-neutral-subtle)" />}
        ...
        {/* CTA */}
        <div style={{ padding: 'var(--seed-dimension-x6) var(--seed-dimension-spacing-x-global-gutter) 0' }}>
          {place.collected ? (
            <Card padding={14} radius={14} style={{
              background: 'var(--seed-color-palette-green-200)', border: '1px solid var(--seed-color-palette-green-300)',
              display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3)',
            }}>
              <span style={{
                width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x9)', borderRadius: 'var(--seed-radius-r3)',
                background: 'var(--seed-color-palette-static-white)',
                display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
              }}><I n="check" s={18} c="var(--seed-color-palette-green-700)" w={2.5} /></span>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-green-900)' }}>
                  이미 도감에 있어요
                </div>
                <div style={{
                  fontSize: 'var(--seed-font-size-t2)', color: 'var(--seed-color-fg-positive)', fontWeight: 'var(--seed-font-weight-medium)', marginTop: 'var(--seed-dimension-x0_5)',
                }}>
                  {place.date}에 발견했어요
                </div>
              </div>
            ...
            <Button variant="primary" size="large" fullWidth
              ...
              trailing={<I n="arrow-right" s={18} c="var(--seed-color-palette-static-white)" />}>
              여기로 떠나기
            </Button>
          )}
        </div>
...
function CongestionCalendar({ data }: { data: CongestionDay[] }) {
  const dows = ['일', '월', '화', '수', '목', '금', '토'];
  const startDow = data[0]?.dow ?? 0;
  const cells: (CongestionDay & { idx: number } | null)[] = [];
  for (let i = 0; i < startDow; i++) cells.push(null);
  data.forEach((d, i) => cells.push({ ...d, idx: i }));
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 'var(--seed-dimension-x1_5)', marginBottom: 'var(--seed-dimension-x1_5)' }}>
        {dows.map((d, i) => (
          <div key={d} style={{
            textAlign: 'center', fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)',
            color: i === 0 ? 'var(--seed-color-fg-critical)' : i === 6 ? 'var(--seed-color-fg-brand)' : 'var(--seed-color-fg-neutral-subtle)',
            }}>{d}</div>
        ))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 'var(--seed-dimension-x1_5)' }}>
        {cells.map((c, i) => c ? (
          <div key={i} style={{
            aspectRatio: '1',
            background: c.band === 0 ? 'var(--seed-color-palette-green-400)' : c.band === 1 ? 'var(--seed-color-palette-yellow-300)' : 'var(--seed-color-palette-red-400)',
            borderRadius: 'var(--seed-radius-r2)',
            display: 'flex', flexDirection: 'column',
            alignItems: 'center', justifyContent: 'center',
            position: 'relative',
            border: c.idx === 0 ? '2px solid var(--seed-color-fg-neutral)' : 'none',
          }}>
            <div style={{
              fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
              lineHeight: 1,
              fontVariantNumeric: 'tabular-nums',
            }}>{c.date.getDate()}</div>
            {c.idx === 0 && (
              <div style={{
                ...
                fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
                }}>오늘</div>
            )}
          </div>
        ) : (
...
function LegendChip({ color, label }: LegendChipProps) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1_5)' }}>
      <div style={{ width: 'var(--seed-dimension-x3_5)', height: 'var(--seed-dimension-x3_5)', borderRadius: 'var(--seed-radius-r1)', background: color }} />
      <span style={{
        fontSize: 'var(--seed-font-size-t2)', color: 'var(--seed-color-fg-neutral-muted)',
        fontWeight: 'var(--seed-font-weight-medium)', }}>{label}</span>
    </div>
  );
}

function InfoStat({ label, value }: LabelValueProps) {
  return (
    <div style={{ flex: 1, textAlign: 'center' }}>
      ...
      <div style={{
        marginTop: 'var(--seed-dimension-x1)', fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)',
        color: 'var(--seed-color-fg-neutral)', }}>{value}</div>
    </div>
  );
}

function InfoStatDivider() {
  return <div style={{ width: 1, background: 'var(--seed-color-stroke-neutral-subtle)', alignSelf: 'stretch' }} />;
}

function InfoRow({ label, value, last }: InfoRowProps) {
  return (
    <div style={{
      display: 'flex', padding: 'var(--seed-dimension-x3_5) var(--seed-dimension-x4)',
      borderBottom: last ? 'none' : '1px solid var(--seed-color-stroke-neutral-subtle)',
    }}>
      <div style={{
        width: 80, fontSize: 'var(--seed-font-size-t3)', color: 'var(--seed-color-fg-neutral-subtle)',
        fontWeight: 'var(--seed-font-weight-medium)', }}>{label}</div>
      <div style={{
        flex: 1, fontSize: 'var(--seed-font-size-t4)', color: 'var(--seed-color-fg-neutral)',
        fontWeight: 'var(--seed-font-weight-bold)', }}>{value}</div>
    </div>
  );
}
...
// ─────────────────────────────────────────────
function PlaceCardModal({ place, region, category, group, idx, userPhoto, onClose }: PlaceCardModalProps) {
  ...
  return (
    <div style={{
      ...
      background: 'var(--seed-color-palette-static-black-alpha-1000)',
      backdropFilter: 'blur(12px)',
      display: 'flex', flexDirection: 'column', overflow: 'hidden',
      animation: 'seed-enter 200ms var(--seed-timing-function-enter)',
      '--seed-enter-opacity': 0, '--seed-enter-translate-y': '10px',
    } as SeedCSSProperties} onClick={onClose}>
      <div style={{
        padding: 'var(--seed-dimension-x14) var(--seed-dimension-x4) 0', display: 'flex', justifyContent: 'space-between',
      }} onClick={e => e.stopPropagation()}>
        <div style={{
          padding: 'var(--seed-dimension-x1_5) var(--seed-dimension-x3)', borderRadius: 'var(--seed-radius-full)',
          background: 'var(--seed-color-palette-static-white-alpha-100)', color: 'var(--seed-color-palette-static-white)',
          fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', }}>{flipped ? '뒷면' : '앞면'}</div>
        <IconButton variant="inverse" onClick={onClose} ariaLabel="닫기"
          style={{ background: 'var(--seed-color-palette-static-white-alpha-200)' }}>
          <I n="close" s={20} c="var(--seed-color-palette-static-white)" />
        </IconButton>
      </div>

      <div style={{
        flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center',
        perspective: 1400, padding: 'var(--seed-dimension-x6)',
      }} onClick={(e) => e.stopPropagation()}>
        <div
          {...clickable(() => setFlipped(f => !f))}
          style={{
            width: cardSize, height: cardSize * 1.4,
            position: 'relative',
            transformStyle: 'preserve-3d',
            transform: `rotateY(${rotY}deg) scale(${entered ? 1 : 'var(--seed-scale-s95)'})`,
          ...
          <CardFace group={group}>
            ...
            <div style={{ padding: 'var(--seed-dimension-x4_5) var(--seed-dimension-x4_5) var(--seed-dimension-x4)' }}>
              <div style={{
                fontSize: 'var(--seed-font-size-t8)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
                }}>{place.name}</div>
              <div style={{
                marginTop: 'var(--seed-dimension-x1_5)', fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)',
                ...
                <span style={{ color: 'var(--seed-color-palette-gray-600)' }}>·</span>
                <span>{region.full}</span>
              </div>
              <div style={{
                marginTop: 'var(--seed-dimension-x3_5)', paddingTop: 'var(--seed-dimension-x3)',
                borderTop: '1px dashed var(--seed-color-stroke-neutral-muted)',
                display: 'flex', justifyContent: 'space-between',
              }}>
          ...
          <CardFace back group={group}>
            <div style={{
              ...
              <div style={{
                fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)',
                color: 'var(--seed-color-fg-neutral-subtle)', }}>모두립 도감 카드</div>
              <div style={{
                marginTop: 'var(--seed-dimension-x1)', fontFamily: 'ui-monospace, monospace',
                fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: group.color,
                }}>#{String(idx).padStart(4, '0')}</div>

              <div style={{
                marginTop: 'auto', marginBottom: 'auto',
                textAlign: 'center', padding: '0 var(--seed-dimension-x3)',
              }}>
                <div style={{
                  display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                  width: 72, height: 72, borderRadius: 'var(--seed-radius-r5)',
                ...
                <div style={{
                  fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)',
                  color: 'var(--seed-color-fg-neutral-subtle)', }}>당신은</div>
                <div style={{
                  marginTop: 'var(--seed-dimension-x1_5)',
                  fontSize: 'var(--seed-font-size-t10)', fontWeight: 'var(--seed-font-weight-bold)',
                  color: 'var(--seed-color-fg-neutral)', lineHeight: 1,
                }}>
                  <span style={{ color: group.color, fontVariantNumeric: 'tabular-nums' }}>{discoverRank.toLocaleString()}</span>
                  <span style={{ fontSize: 'var(--seed-font-size-t6)', fontWeight: 'var(--seed-font-weight-bold)' }}>번째</span>
                </div>
                <div style={{
                  marginTop: 'var(--seed-dimension-x1)',
                  fontSize: 'var(--seed-font-size-t5)', fontWeight: 'var(--seed-font-weight-bold)',
                  color: 'var(--seed-color-fg-neutral)', }}>발견자입니다</div>
                <div style={{
                  marginTop: 'var(--seed-dimension-x3_5)',
                  fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)',
                  color: 'var(--seed-color-fg-neutral-muted)', }}>지금까지 <b style={{ color: 'var(--seed-color-fg-neutral)' }}>{totalDiscover.toLocaleString()}명</b>이 이곳을 도감에 담았어요</div>
              </div>

              <div style={{
                paddingTop: 'var(--seed-dimension-x3_5)', borderTop: '1px dashed var(--seed-color-stroke-neutral-muted)',
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1_5)' }}>
                  <I n="map-pin" s={12} c="var(--seed-color-fg-neutral-subtle)" />
                  <span style={{
                    fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-muted)',
                    }}>{region.name}</span>
                </div>
                <div style={{
                  fontFamily: 'ui-monospace, monospace', fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)',
                  color: 'var(--seed-color-fg-neutral-subtle)', }}>모두립 · 2026</div>
              </div>
            </div>
      ...
      <div style={{
        padding: '0 var(--seed-dimension-x6) var(--seed-dimension-x10)', textAlign: 'center',
        fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)',
        color: 'var(--seed-color-palette-static-white-alpha-600)', }}>카드를 탭해서 뒤집어 보세요</div>
    </div>
  );
}

function CardFace({ children, back, group }: CardFaceProps) {
  return (
    <div style={{
      position: 'absolute', inset: 0,
      borderRadius: 'var(--seed-radius-r6)', overflow: 'hidden',
      background: 'var(--seed-color-palette-static-white)',
      boxShadow: `
        0 0 0 4px ${(back && group) ? group.color : (group?.color || 'var(--seed-color-bg-brand-solid)')},
        0 0 0 5px var(--seed-color-palette-static-white-alpha-500),
        0 30px 60px var(--seed-color-palette-static-black-alpha-700),
        0 12px 24px var(--seed-color-palette-static-black-alpha-700)
      `,
...
function CardHolo({ shineX, shineY, rotY, intensity }: CardHoloProps) {
  return (
    <>
      <div style={{
        position: 'absolute', inset: 0, pointerEvents: 'none',
        background: `radial-gradient(circle at ${shineX}% ${shineY}%,
          var(--seed-color-palette-static-white-alpha-500) 0%,
          var(--seed-color-palette-static-white-alpha-300) 20%,
          rgba(255,255,255,0) 45%)`,
        mixBlendMode: 'overlay', opacity: 0.9,
      }} />
      <div style={{
        position: 'absolute', inset: 0, pointerEvents: 'none',
        // 절차적 홀로그램 - 알파가 기울기(intensity)로 런타임 계산되므로
        // 고정 토큰으로 표현할 수 없다. SEED 미대응 축이라 rgba 를 남긴다.
...
function CardEdge({ intensity }: CardEdgeProps) {
  return (
    <div style={{
      position: 'absolute', inset: 0, pointerEvents: 'none',
...
function CardStat({ label, value, mono }: LabelValueProps & { mono?: boolean }) {
  return (
    <div style={{ textAlign: 'center', minWidth: 0 }}>
      ...
      <div style={{
        marginTop: 'var(--seed-dimension-x1)', fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)',
        color: 'var(--seed-color-fg-neutral)', fontFamily: mono ? 'ui-monospace, monospace' : undefined,
        fontVariantNumeric: 'tabular-nums',
      }}>{value}</div>
    </div>
  );
    ...
    <div style={{ width: '100%', height: '100%', position: 'relative', background: 'var(--seed-color-palette-gray-300)', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', top: 56, left: 16, right: 64, zIndex: 5 }}>
        <div style={{
          ...
          <div style={{
            flex: 1, fontSize: 'var(--seed-font-size-t4)', color: 'var(--seed-color-fg-neutral-muted)',
            fontWeight: 'var(--seed-font-weight-medium)', }}>주변 자원 찾기</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--seed-dimension-x2)',
      }}>
        <div style={{
          ...
          <button
            ...
            aria-label="확대"
            style={{
              width: 'var(--seed-dimension-x10)', height: 'var(--seed-dimension-x10)', border: 'none',
              background: 'transparent', cursor: canZoomIn ? 'pointer' : 'not-allowed',
              color: canZoomIn ? 'var(--seed-color-fg-neutral)' : 'var(--seed-color-fg-disabled)',
          ...
          <button
            ...
            style={{
              ...
              color: canZoomOut ? 'var(--seed-color-fg-neutral)' : 'var(--seed-color-fg-disabled)',
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
              WebkitTapHighlightColor: 'transparent',
            }}>
        ...
        <div style={{
          ...
          background: 'var(--seed-color-bg-neutral-inverted)', color: 'var(--seed-color-fg-neutral-inverted)',
      ...
      <div style={{
        ...
        <div style={{
          width: 'var(--seed-dimension-x5)', height: 'var(--seed-dimension-x5)', borderRadius: '50%',
          background: 'var(--seed-color-bg-brand-solid)', border: '3px solid var(--seed-color-palette-static-white)',
          boxShadow: '0 0 0 2px var(--seed-color-stroke-brand-solid), var(--seed-shadow-s2)',
        }} />
      </div>
      ...
      <div style={{
        ...
        padding: 'var(--seed-dimension-x2_5) var(--seed-dimension-x3_5)', color: 'var(--seed-color-fg-neutral-inverted)',
        display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x2)',
        fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)', }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', padding: 'var(--seed-dimension-x2) 0 var(--seed-dimension-x1)' }}>
          <div style={{ width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x1)', borderRadius: 'var(--seed-radius-r1)', background: 'var(--seed-color-palette-gray-400)' }} />
        </div>
        <div style={{ padding: 'var(--seed-dimension-x2) var(--seed-dimension-x4) var(--seed-dimension-x3_5)', display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3)' }}>
          <div style={{
            width: 'var(--seed-dimension-x14)', height: 'var(--seed-dimension-x14)', borderRadius: 'var(--seed-radius-r3)', overflow: 'hidden', flexShrink: 0,
          }}>
            <PhotoPlaceholder hue={place.hue} lift={place.lift}
              silhouette={!place.collected}
              style={{ width: '100%', height: '100%' }} />
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', gap: 'var(--seed-dimension-x1)', marginBottom: 'var(--seed-dimension-x1)', flexWrap: 'wrap' }}>
              <Badge tone={group.id === 'culture' ? 'accent' : group.id === 'festival' ? 'festival' : group.id === 'heritage' ? 'heritage' : 'success'}
        ...
        <div style={{ padding: '0 var(--seed-dimension-x3_5) var(--seed-dimension-x3_5)' }}>
          {place.collected ? (
            <Card padding={12} radius={12} style={{
              background: 'var(--seed-color-palette-green-200)', border: '1px solid var(--seed-color-palette-green-300)',
              display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x2_5)',
            }}>
              <I n="check" s={18} c="var(--seed-color-palette-green-700)" w={2.5} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-green-900)' }}>이미 발견했어요</div>
                <div style={{ fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-positive)' }}>
                  {place.date} 도감에 기록됨
                </div>
              </div>
            </Card>
...
function DiscoverSuccessScreen({ placeId, onDone, onWriteReview }: DiscoverSuccessScreenProps) {
  ...
  return (
    <div style={{
      position: 'absolute', inset: 0, zIndex: 200,
      background: stage >= 2 ? 'var(--seed-color-palette-static-white)' : 'var(--seed-color-bg-neutral-inverted)',
      color: stage >= 2 ? 'var(--seed-color-fg-neutral)' : 'var(--seed-color-fg-neutral-inverted)',
      transition: 'background 400ms, color 400ms',
      display: 'flex', flexDirection: 'column', overflow: 'hidden',
    }}>
      {stage >= 2 && <Confetti />}
      <div style={{ padding: '72px var(--seed-dimension-x6) 0', textAlign: 'center' }}>
        <div style={{
          fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)',
        ...
        <div style={{
          ...
          fontSize: 'var(--seed-font-size-t9)',
          lineHeight: 'var(--seed-line-height-t9)',
          fontWeight: 'var(--seed-font-weight-bold)', opacity: stage >= 1 ? 1 : 0,
          transition: 'opacity var(--seed-duration-d6)',
        }}>{stage >= 2 ? `${place.name}을\n도감에 추가했어요` : place.name}</div>
      </div>

      <div style={{
        flex: '0 0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center',
        padding: 'var(--seed-dimension-x6) var(--seed-dimension-x10) var(--seed-dimension-x2)',
        perspective: 1100,
      }}>
        <div style={{
          width: 220, height: 290, borderRadius: 'var(--seed-radius-r5)',
          position: 'relative', overflow: 'hidden',
          border: `1px solid ${stage >= 2 ? group.color : 'var(--seed-color-palette-static-white-alpha-200)'}`,
          background: 'var(--seed-color-palette-static-white)',
          boxShadow: stage >= 2
            ? 'var(--seed-shadow-s2)'
            : 'var(--seed-shadow-s3)',
          ...
          transition:
            stage === 1
              ? 'transform 1100ms var(--seed-timing-function-easing), border-color 400ms, box-shadow 400ms'
              : 'transform 500ms var(--seed-timing-function-enter-expressive), border-color 400ms, box-shadow 400ms',
        }}>
          <div style={{ height: 180, position: 'relative' }}>
            ...
            {stage === 0 && (
              <div style={{
                ...
                background: 'linear-gradient(90deg, transparent, var(--seed-color-bg-brand-solid), transparent)',
                animation: 'seed-exit 1s var(--seed-timing-function-linear) infinite',
                '--seed-exit-translate-y': '180px',
                boxShadow: '0 0 12px var(--seed-color-bg-brand-solid)',
              } as SeedCSSProperties} />
            )}
            {stage >= 1 && (
              <div style={{
                position: 'absolute', top: 12, right: 12,
                animation: 'seed-enter 300ms var(--seed-timing-function-enter)',
                '--seed-enter-opacity': 0, '--seed-enter-translate-y': '10px',
              } as SeedCSSProperties}>
                <Badge tone="success" variant="solid" size="medium"
                  leading={<I n="check" s={12} c="var(--seed-color-palette-static-white)" w={2.5} />}>
                  발견
                </Badge>
              </div>
            )}
          </div>
          <div style={{ padding: 'var(--seed-dimension-x3_5)' }}>
            <Badge tone={group.id === 'culture' ? 'accent' : group.id === 'festival' ? 'festival' : group.id === 'heritage' ? 'heritage' : 'success'}
              variant="subtle" size="xsmall">
              {group.label} · {category.label}
            </Badge>
            ...
            <div style={{
              fontFamily: 'ui-monospace, monospace', fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-medium)',
              color: 'var(--seed-color-fg-neutral-subtle)', marginTop: 'var(--seed-dimension-x1_5)',
        ...
        <>
          ...
          <div style={{ padding: 'var(--seed-dimension-x3) var(--seed-dimension-x6) 0', animation: 'seed-enter 320ms var(--seed-timing-function-enter) 180ms both',
            ...
            <Card padding={14} radius={14}>
              <div style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--seed-dimension-x2)',
              }}>
                <span style={{
                  fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
                  }}>{region.name} · {group.label}</span>
                <span style={{
                  fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', color: group.color,
                  fontVariantNumeric: 'tabular-nums',
          ...
          <div style={{
            padding: '0 var(--seed-dimension-x6) var(--seed-dimension-x8)',
            display: 'flex', gap: 'var(--seed-dimension-x2_5)',
            animation: 'seed-enter 320ms var(--seed-timing-function-enter) 260ms both',
            '--seed-enter-opacity': 0, '--seed-enter-translate-y': '10px',
          } as SeedCSSProperties}>
...
// Titles - 5 tiers
// ─────────────────────────────────────────────
function TitlesScreen({ onBack }: TitlesScreenProps) {
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)' }}>
      ...
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-x4)' }}>
        <Card padding={16} radius={16}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3_5)' }}>
            <span style={{
              width: 'var(--seed-dimension-x12)', height: 'var(--seed-dimension-x12)', borderRadius: 'var(--seed-radius-r3)',
              background: 'var(--seed-color-palette-yellow-100)',
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            }}><I n="trophy" s={24} c="var(--seed-color-palette-yellow-700)" w={2} /></span>
            <div style={{ flex: 1 }}>
              <div style={{
                fontSize: 'var(--seed-font-size-t8)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
                ...
                {earned.length}<span style={{
                  fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)',
                }}> / {TITLES.length}</span>
              </div>
              <div style={{
                fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-subtle)',
                marginTop: 'var(--seed-dimension-x1)', }}>획득한 칭호 · 5티어 시스템</div>
            </div>
      ...
      <div style={{
        padding: '0 var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-x5)',
        display: 'flex', gap: 'var(--seed-dimension-x1_5)', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none',
      }}>
        {tierOrder.map(tid => {
          ...
          return (
            <div key={tid} style={{
              flexShrink: 0,
              padding: 'var(--seed-dimension-x2) var(--seed-dimension-x3)', borderRadius: 'var(--seed-radius-r3)',
              background: 'var(--seed-color-palette-static-white)',
              ...
              <span style={{
                width: 'var(--seed-dimension-x1_5)', height: 'var(--seed-dimension-x4_5)', borderRadius: 'var(--seed-radius-r1)', background: t.color,
              }} />
              <span style={{
      ...
      {tierOrder.map(tid => {
        ...
        return (
          <div key={tid} style={{ marginBottom: 'var(--seed-dimension-x6)' }}>
            <SectionHeader title={tier.label} subtitle={`${tier.label} 티어 · ${items.length}개`} dense />
            <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)', display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x2)' }}>
  ...
  if (title.earned) {
    return (
      <Card padding={14} radius={14}
        ...
        <div style={{ flex: 1 }}>
          <div style={{
            fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)', color: tier.color,
            }}>{title.name}</div>
          <div style={{
            fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-muted)',
            marginTop: 'var(--seed-dimension-x0_5)', }}>{title.desc}</div>
        </div>
  ...
  return (
    <Card padding={14} radius={14}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3)' }}>
        <span style={{
          width: 'var(--seed-dimension-x10)', height: 'var(--seed-dimension-x10)', borderRadius: 'var(--seed-radius-r3)',
          background: 'var(--seed-color-bg-neutral-weak)', color: 'var(--seed-color-fg-neutral-muted)',
          display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
        }}><I n={title.icon} s={20} c="var(--seed-color-fg-neutral-subtle)" /></span>
        <div style={{ flex: 1 }}>
          <div style={{
            ...
            <span style={{
              fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', color: tier.color,
              fontVariantNumeric: 'tabular-nums',
            }}>{title.progress ?? 0}/{title.total ?? 0}</span>
          </div>
          <div style={{ marginTop: 'var(--seed-dimension-x1_5)' }}>
            <Progress value={title.progress ?? 0} total={title.total ?? 0} color={tier.color} height={4} />
          </div>
          <div style={{
            fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-subtle)',
            marginTop: 'var(--seed-dimension-x1_5)', }}>{title.desc}</div>
        </div>
      </div>
    </Card>
  );
...
// ─────────────────────────────────────────────
const CONFETTI_COLORS = ['var(--seed-color-bg-brand-solid)', 'var(--seed-color-fg-positive)', 'var(--seed-color-fg-warning)', 'var(--seed-color-palette-purple-700)', 'var(--seed-color-palette-red-700)', 'var(--seed-color-fg-informative)', 'var(--seed-color-palette-yellow-300)', 'var(--seed-color-fg-critical)'];

function confettiUnit(seed: number): number {
  const value = Math.sin(seed * 12.9898) * 43758.5453;
  return value - Math.floor(value);
}

const CONFETTI_PIECES = Array.from({ length: 56 }, (_, i) => ({
  id: i,
  x: confettiUnit(i * 6 + 1) * 100,
  delay: confettiUnit(i * 6 + 2) * 0.8,
  dur: 2.4 + confettiUnit(i * 6 + 3) * 2.2,
  size: 6 + confettiUnit(i * 6 + 4) * 8,
  rot: confettiUnit(i * 6 + 5) * 360,
  drift: (confettiUnit(i * 6 + 6) - 0.5) * 80,
    ...
    <div style={{
      ...
      {CONFETTI_PIECES.map(p => (
        <div key={p.id} style={{
          ...
          opacity: 0.92,
          animation: `seed-exit ${p.dur}s var(--seed-timing-function-exit) ${p.delay}s forwards`,
          '--seed-exit-translate-x': `${p.drift}px`,
          '--seed-exit-translate-y': '130vh',
          '--seed-exit-rotate': '720deg',
          '--seed-exit-opacity': 0,
        } as SeedCSSProperties} />
      ))}
    </div>
...
function PlazaScreen({ onOpenPlace, onOpenPreset, onOpenUser }: PlazaScreenProps) {
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)' }}>
      ...
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-x4)' }}>
        <div style={{
          background: 'var(--seed-color-bg-neutral-weak)', borderRadius: 'var(--seed-radius-r3)', padding: 'var(--seed-dimension-x1)',
          display: 'flex',
        }}>
          {[
            { id: 'feed', label: '피드' },
            ...
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              flex: 1, padding: 'var(--seed-dimension-x2_5) var(--seed-dimension-x1_5)', borderRadius: 'var(--seed-radius-r2)',
              background: tab === t.id ? 'var(--seed-color-palette-static-white)' : 'transparent',
              color: tab === t.id ? 'var(--seed-color-fg-neutral)' : 'var(--seed-color-fg-neutral-subtle)',
              border: 'none', cursor: 'pointer',
              fontWeight: 'var(--seed-font-weight-bold)', fontSize: 'var(--seed-font-size-t2)',
              transition: 'all var(--seed-duration-d2)',
            }}>{t.label}</button>
          ))}
        </div>
...
function PlazaPresets({ onOpenPreset, onOpenUser }: PlazaPresetsProps) {
  const [query, setQuery] = React.useState('');
  ...
  return (
    <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)', display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x3)' }}>
      ...
      <div style={{
        display: 'flex', gap: 'var(--seed-dimension-x1_5)', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none',
        paddingBottom: 'var(--seed-dimension-x1)',
      }}>
      ...
      {activeTags.length === 0 && !query && (
        <Card padding={16} radius={16} style={{
          ...
          <div style={{
            fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-brand)',
            }}>이번 주 인기 코스</div>
          <div style={{
            marginTop: 'var(--seed-dimension-x1)',
            fontSize: 'var(--seed-font-size-t7)',
      ...
      {(activeTags.length > 0 || query) && (
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          padding: 'var(--seed-dimension-x1) var(--seed-dimension-x0_5)',
        }}>
          <span style={{
            fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
          ...
          <button onClick={() => { setActiveTags([]); setQuery(''); }} style={{
            ...
            fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-brand)',
            }}>필터 초기화</button>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <Card padding={24} radius={14} style={{
          background: 'var(--seed-color-bg-layer-basement)', border: '1px dashed var(--seed-color-palette-gray-500)',
          textAlign: 'center',
        }}>
          <div style={{
            fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-subtle)',
          ...
          <div key={p.id} style={{ position: 'relative' }}>
            {activeTags.length === 0 && !query && i < 3 && (
              <div style={{
                ...
                background: i === 0 ? 'var(--seed-color-palette-yellow-300)' : i === 1 ? 'var(--seed-color-palette-gray-500)' : 'var(--seed-color-palette-carrot-800)',
                color: 'var(--seed-color-palette-static-white)',
                display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', boxShadow: '0 2px 6px var(--seed-color-palette-static-black-alpha-400)',
                fontVariantNumeric: 'tabular-nums',
              }}>{i + 1}</div>
            )}
            <PresetCard
...
function PlazaFeed({ onOpenPlace }: PlazaFeedProps) {
  return (
    <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)', display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x3)' }}>
      <div style={{
        display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1_5)',
        ...
        <span style={{
          width: 'var(--seed-dimension-x1_5)', height: 'var(--seed-dimension-x1_5)', borderRadius: '50%', background: 'var(--seed-color-palette-green-700)',
          animation: 'seed-exit 750ms var(--seed-timing-function-easing) infinite alternate',
          '--seed-exit-opacity': 0.25, display: 'inline-block',
        } as SeedCSSProperties} />
        실시간 발견
      </div>
      ...
      {FEED.map(f => {
        ...
        return (
          <Card key={f.id} padding={16} radius={16}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x2_5)', marginBottom: 'var(--seed-dimension-x3)' }}>
              <Avatar initial={f.initial} size={36} color={f.color} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1_5)', flexWrap: 'wrap',
                }}>
                  <span style={{
                    fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
                    }}>{f.user}</span>
                ...
                <div style={{
                  fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-subtle)',
                  marginTop: 'var(--seed-dimension-x0_5)', }}>{f.region} · {f.timeAgo}</div>
              </div>
            </div>

            <div {...clickable(() => onOpenPlace(place.id))} style={{
              position: 'relative', borderRadius: 'var(--seed-radius-r4)', overflow: 'hidden', cursor: 'pointer',
              height: 152, border: '1px solid var(--seed-color-stroke-neutral-subtle)',
            }}>
              <PhotoPlaceholder hue={place.hue} lift={place.lift}
                style={{ width: '100%', height: '100%' }} />
              <div style={{
                position: 'absolute', left: 0, right: 0, bottom: 0,
                padding: 'var(--seed-dimension-x9) var(--seed-dimension-x3_5) var(--seed-dimension-x3)',
                background: 'linear-gradient(180deg, transparent, var(--seed-color-palette-static-black-alpha-800))',
              }}>
                <div style={{
                  fontSize: 'var(--seed-font-size-t5)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-static-white)',
                  }}>{place.name}</div>
                <div style={{
                  fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-static-white-alpha-800)',
                  marginTop: 'var(--seed-dimension-x0_5)', }}>{cat.label}</div>
              </div>
            ...
            {f.titleEarned && (
              <div style={{
                marginTop: 'var(--seed-dimension-x3)', padding: 'var(--seed-dimension-x2_5) var(--seed-dimension-x3)', borderRadius: 'var(--seed-radius-r3)',
                background: 'var(--seed-color-palette-yellow-100)', border: '1px solid var(--seed-color-palette-yellow-200)',
                display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x2)',
                fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-yellow-900)',
              }}>
                <I n="trophy" s={14} c="var(--seed-color-palette-yellow-700)" />
                <span><b>{f.titleEarned}</b> 칭호 획득</span>
              </div>
            )}

            <div style={{
              display: 'flex', gap: 'var(--seed-dimension-x5)', marginTop: 'var(--seed-dimension-x3_5)', paddingTop: 'var(--seed-dimension-x3)',
              borderTop: '1px solid var(--seed-color-stroke-neutral-subtle)',
            }}>
...
function FeedAction({ iconName, label, count }: FeedActionProps) {
  return (
    <button style={{
      background: 'transparent', border: 'none', padding: 0, cursor: 'pointer',
      display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1_5)',
      fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-muted)',
      fontVariantNumeric: 'tabular-nums',
    }}>
...
function PlazaLeaders() {
  const [metric, setMetric] = React.useState<'diversity' | 'count'>('diversity');
  const sorted = [...LEADERS].sort((a, b) =>
    metric === 'diversity' ? b.diversity - a.diversity : b.count - a.count
  );

  return (
    <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)', display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x3_5)' }}>
      <Card padding={16} radius={16}>
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        }}>
        ...
        <div style={{
          marginTop: 'var(--seed-dimension-x3_5)', padding: 'var(--seed-dimension-x1)', borderRadius: 'var(--seed-radius-r3)',
          background: 'var(--seed-color-bg-neutral-weak)', display: 'flex',
        }}>
          {([
            ...
            <button key={m.id} onClick={() => setMetric(m.id)} style={{
              flex: 1, padding: 'var(--seed-dimension-x2) var(--seed-dimension-x2_5)', borderRadius: 'var(--seed-radius-r2)',
              background: metric === m.id ? 'var(--seed-color-palette-static-white)' : 'transparent',
              color: metric === m.id ? 'var(--seed-color-fg-neutral)' : 'var(--seed-color-fg-neutral-subtle)',
              border: 'none', cursor: 'pointer',
              fontWeight: 'var(--seed-font-weight-bold)', fontSize: 'var(--seed-font-size-t2)',
            }}>{m.label}</button>
          ))}
      ...
      <div style={{
        display: 'grid', gridTemplateColumns: '1fr 1.2fr 1fr', gap: 'var(--seed-dimension-x2)', alignItems: 'end',
      }}>
        {[sorted[1]!, sorted[0]!, sorted[2]!].map((l, i) => (
          <PodiumCard key={l.rank} leader={l} highlight={i === 1}
            podium={([2, 1, 3] as const)[i]!} metric={metric} />
        ))}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x2)' }}>
        {sorted.slice(3).map((l, idx) => (
          <Card key={l.rank} padding={12} radius={12}
            style={l.me ? {
              background: 'var(--seed-color-bg-brand-weak)', border: '1px solid var(--seed-color-stroke-brand-solid)',
            } : undefined}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3)' }}>
              <span style={{
                width: 'var(--seed-dimension-x7)', fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)', textAlign: 'center',
                color: l.me ? 'var(--seed-color-fg-brand)' : 'var(--seed-color-fg-neutral-subtle)',
                fontVariantNumeric: 'tabular-nums',
              }}>{idx + 4}</span>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{
                  fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-subtle)',
                  marginTop: 'var(--seed-dimension-x0_5)', }}>{l.title}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{
                  ...
                  <span style={{
                    fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)', marginLeft: 'var(--seed-dimension-x0_5)',
...
function PodiumCard({ leader, highlight, podium, metric }: PodiumCardProps) {
  const colors = { 1: 'var(--seed-color-palette-yellow-300)', 2: 'var(--seed-color-palette-gray-500)', 3: 'var(--seed-color-palette-carrot-800)' };
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{
        marginTop: 'var(--seed-dimension-x1_5)', fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
        }}>{leader.name}</div>
      <div style={{
        fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)',
        marginTop: 'var(--seed-dimension-x0_5)', fontVariantNumeric: 'tabular-nums',
      ...
      <div style={{
        marginTop: 'var(--seed-dimension-x2_5)',
        height, borderRadius: '10px 10px 0 0',
        background: medal,
        display: 'flex', alignItems: 'flex-start', justifyContent: 'center',
        paddingTop: 'var(--seed-dimension-x2_5)',
        fontSize: 'var(--seed-font-size-t8)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-static-white)',
        }}>{podium}</div>
    </div>
  );
}
...
function PlazaCompanion() {
  ...
  return (
    <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)', display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x3)' }}>
      <div style={{
        background: 'var(--seed-color-bg-neutral-weak)', borderRadius: 'var(--seed-radius-r3)', padding: 'var(--seed-dimension-x1)', display: 'flex',
      }}>
        <button onClick={() => setTab('light')} style={{
          flex: 1, padding: 'var(--seed-dimension-x2) var(--seed-dimension-x2_5)', borderRadius: 'var(--seed-radius-r2)',
          background: tab === 'light' ? 'var(--seed-color-palette-static-white)' : 'transparent',
          color: tab === 'light' ? 'var(--seed-color-fg-neutral)' : 'var(--seed-color-fg-neutral-subtle)',
          border: 'none', cursor: 'pointer',
          fontWeight: 'var(--seed-font-weight-bold)', fontSize: 'var(--seed-font-size-t2)',
        }}>비-산 (가벼움)</button>
        <button onClick={() => setTab('mountain')} style={{
          flex: 1, padding: 'var(--seed-dimension-x2) var(--seed-dimension-x2_5)', borderRadius: 'var(--seed-radius-r2)',
          background: tab === 'mountain' ? 'var(--seed-color-palette-static-white)' : 'transparent',
          color: tab === 'mountain' ? 'var(--seed-color-fg-neutral)' : 'var(--seed-color-fg-neutral-subtle)',
          border: 'none', cursor: 'pointer',
          fontWeight: 'var(--seed-font-weight-bold)', fontSize: 'var(--seed-font-size-t2)',
        }}>산악 (안전 우선)</button>
      </div>
      ...
      {tab === 'mountain' && (
        <Card padding={12} radius={12} style={{
          ...
          <div style={{
            ...
            lineHeight: 1.55, }}>
            <I n="info" s={14} c="var(--seed-color-palette-yellow-700)" style={{ flexShrink: 0, marginTop: 'var(--seed-dimension-x0_5)' }} />
            <span>산악 동행은 <b style={{ fontWeight: 'var(--seed-font-weight-bold)' }}>안전 우선 태그</b>가 필수입니다. 등산 경력·코스·장비를 명시해주세요.</span>
          </div>
        </Card>
      )}

      <Card padding={16} radius={16} style={{
        background: 'var(--seed-color-palette-blue-100)', border: '1px dashed var(--seed-color-stroke-brand-solid)',
      }}>
        <div style={{
          fontSize: 'var(--seed-font-size-t5)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
          }}>{tab === 'mountain' ? '산악 동행 모집' : '동행 모집'}</div>
        <div style={{
          fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-muted)', marginTop: 'var(--seed-dimension-x1)',
...
function CompanionCard({ c, mountain }: CompanionCardProps) {
  return (
    <Card padding={16} radius={16}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3)' }}>
        <Avatar initial={c.initial} size={44} color={c.color} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1_5)', flexWrap: 'wrap' }}>
            <span style={{
              fontSize: 'var(--seed-font-size-t5)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
              }}>{c.name}</span>
            <Badge tone="primary" variant="subtle" size="xsmall">
      ...
      <div style={{
        marginTop: 'var(--seed-dimension-x3)', padding: 'var(--seed-dimension-x2_5) var(--seed-dimension-x3)', borderRadius: 'var(--seed-radius-r3)',
        background: 'var(--seed-color-bg-neutral-weak)',
        display: 'flex', flexWrap: 'wrap', gap: 'var(--seed-dimension-x3_5)',
        fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-muted)',
      }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x1)' }}>
          <I n="map-pin" s={12} c="var(--seed-color-fg-neutral-subtle)" />
          <b style={{ color: 'var(--seed-color-fg-neutral)', fontWeight: 'var(--seed-font-weight-bold)' }}>
            {c.region}{c.target ? ` · ${c.target}` : ''}
          </b>
        </span>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x1)' }}>
          <I n="calendar" s={12} c="var(--seed-color-fg-neutral-subtle)" />
          <b style={{ color: 'var(--seed-color-fg-neutral)', fontWeight: 'var(--seed-font-weight-bold)' }}>{c.when}</b>
        </span>
      </div>

      {mountain && c.safetyTags && (
        <div style={{ marginTop: 'var(--seed-dimension-x2)', display: 'flex', gap: 'var(--seed-dimension-x1)', flexWrap: 'wrap' }}>
          {c.safetyTags.map(t => (
      ...
      <div style={{
        ...
        fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral)',
      }}>‘{c.looking}’</div>

      <div style={{ marginTop: 'var(--seed-dimension-x3)', display: 'flex', gap: 'var(--seed-dimension-x1_5)', flexWrap: 'wrap' }}>
        {c.titles.map(t => (
      ...
      <div style={{
        marginTop: 'var(--seed-dimension-x3)', paddingTop: 'var(--seed-dimension-x2_5)',
        borderTop: '1px solid var(--seed-color-stroke-neutral-subtle)',
        fontSize: 'var(--seed-font-size-t1)', color: 'var(--seed-color-fg-neutral-subtle)',
        display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1)',
        fontWeight: 'var(--seed-font-weight-medium)', }}>
        <I n="message-circle" s={12} c="var(--seed-color-fg-neutral-subtle)" />
...
function loadMyPresets(): Preset[] {
  try {
    const raw = localStorage.getItem(MY_PRESETS_KEY);
    return raw ? JSON.parse(raw) as Preset[] : [];
  } catch { return []; }
}
function saveMyPresets(list: Preset[]): void {
  try { localStorage.setItem(MY_PRESETS_KEY, JSON.stringify(list)); } catch {}
}
function loadSavedPresets(): string[] {
  try {
    const raw = localStorage.getItem(SAVED_PRESETS_KEY);
    return raw ? JSON.parse(raw) as string[] : [];
  } catch { return []; }
}
function saveSavedPresets(list: string[]): void {
  try { localStorage.setItem(SAVED_PRESETS_KEY, JSON.stringify(list)); } catch {}
}

function useMyPresets(): [Preset[], (next: Preset[]) => void] {
  const [list, setList] = React.useState(loadMyPresets);
  const update = (next: Preset[]) => { setList(next); saveMyPresets(next); };
  return [list, update];
}
function useSavedPresets(): [string[], (next: string[]) => void] {
  const [list, setList] = React.useState(loadSavedPresets);
  const update = (next: string[]) => { setList(next); saveSavedPresets(next); };
  return [list, update];
}
...
// ─────────────────────────────────────────────
function PresetCard({ preset, onClick, onOpenOwner, compact, savedBadge }: PresetCardProps) {
  const region = getRegion(preset.region);
  const placeCount = preset.placeIds.length;
  return (
    <Card padding={0} radius={16} onClick={onClick}>
      <div style={{
        padding: 'var(--seed-dimension-x3_5)', display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3)',
        borderBottom: '1px solid var(--seed-color-stroke-neutral-subtle)',
      }}>
        <span style={{
          width: 'var(--seed-dimension-x10)', height: 'var(--seed-dimension-x10)', borderRadius: 'var(--seed-radius-r3)',
          ...
          {region
            ...
            : <I n="map" s={20} c="var(--seed-color-palette-gray-800)" />}
        </span>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
          ...
          <div style={{
            fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-subtle)',
            marginTop: 'var(--seed-dimension-x0_5)', display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1_5)',
          }}>
            <span>{region ? region.name : ''}</span>
            <span style={{ color: 'var(--seed-color-palette-gray-500)' }}>·</span>
            <span>{placeCount}곳</span>
            {preset.savedCount != null && (
              <>
                <span style={{ color: 'var(--seed-color-palette-gray-500)' }}>·</span>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x0_5)' }}>
                  <I n="bookmark" s={11} c="var(--seed-color-fg-neutral-subtle)" />
      ...
      {!compact && (
        <div style={{ padding: 'var(--seed-dimension-x3)', display: 'flex', gap: 'var(--seed-dimension-x1_5)', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {preset.placeIds.slice(0, 5).map(pid => {
            const p = PLACES.find(x => x.id === pid);
            if (!p) return null;
            return (
              <div key={pid} style={{
                width: 'var(--seed-dimension-x14)', flexShrink: 0,
              }}>
                <div style={{
                  width: 'var(--seed-dimension-x14)', height: 'var(--seed-dimension-x14)', borderRadius: 'var(--seed-radius-r3)', overflow: 'hidden',
                  border: '1px solid var(--seed-color-stroke-neutral-subtle)',
                }}>
                  <PhotoPlaceholder hue={p.hue} lift={p.lift}
                    style={{ width: '100%', height: '100%' }} />
                ...
                <div style={{
                  fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', marginTop: 'var(--seed-dimension-x1)',
                  color: 'var(--seed-color-fg-neutral-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                  textAlign: 'center',
                }}>{p.name}</div>
              </div>
            );
          ...
          {preset.placeIds.length > 5 && (
            <div style={{
              minWidth: 'var(--seed-dimension-x14)', height: 'var(--seed-dimension-x14)', borderRadius: 'var(--seed-radius-r3)',
              background: 'var(--seed-color-bg-neutral-weak)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-muted)',
              flexShrink: 0,
      ...
      {preset.ownerName && (
        <button
          type="button"
          disabled={!preset.ownerId || !onOpenOwner}
          onClick={(event) => {
            event.stopPropagation();
            if (preset.ownerId) onOpenOwner?.(preset.ownerId);
          }}
          style={{
          width: '100%', background: 'transparent', border: 'none', textAlign: 'left',
          padding: 'var(--seed-dimension-x2) var(--seed-dimension-x3_5) var(--seed-dimension-x3)',
          display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1_5)',
          fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)',
          cursor: preset.ownerId && onOpenOwner ? 'pointer' : 'default',
        }}>
          <Avatar initial={preset.ownerInitial} size={18} color={preset.ownerColor || 'var(--seed-color-bg-brand-solid)'} />
...
// ─────────────────────────────────────────────
function PresetCourseMap({ preset, region, onOpenPlace }: PresetCourseMapProps) {
  if (!region) return null;
  ...
  return (
    <Card padding={14} radius={18}>
      <svg viewBox={vb} width="100%" style={{
        display: 'block', aspectRatio: '1', background: 'var(--seed-color-bg-layer-basement)', borderRadius: 'var(--seed-radius-r3)',
      }}>
        {/* region fill */}
        <path d={d}
          fill="var(--seed-color-palette-static-white)" stroke="var(--seed-color-palette-gray-600)"
          strokeWidth={Math.max(b.w, b.h) * 0.004} strokeLinejoin="round" />
        {/* path between pins */}
        <path d={pathD} fill="none"
          stroke="var(--seed-color-stroke-brand-solid)" strokeWidth={Math.max(b.w, b.h) * 0.006}
        ...
        {pinPositions.map(pin => {
          ...
          return (
            <g key={pin.place.id}
              {...clickable(() => onOpenPlace?.(pin.place.id))}
              style={{ cursor: 'pointer' }}>
              {/* pin shadow */}
              <circle cx={pin.x} cy={pin.y + pinR * 0.18}
                r={pinR * 1.05} fill="var(--seed-color-palette-static-black-alpha-500)" />
              {/* pin body */}
              <circle cx={pin.x} cy={pin.y} r={pinR}
                fill={grp.color} stroke="var(--seed-color-palette-static-white)"
      ...
      <div style={{
        marginTop: 'var(--seed-dimension-x3)', paddingTop: 'var(--seed-dimension-x3)', borderTop: '1px dashed var(--seed-color-stroke-neutral-muted)',
        display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x1_5)',
        fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-subtle)',
        }}>
        <I n="info" s={12} c="var(--seed-color-fg-neutral-subtle)" />
        <span>핀을 탭하면 자원 상세로 이동합니다</span>
      </div>
    </Card>
  );
}
function PresetDetailScreen({ presetId, onBack, onOpenPlace, onOpenUser }: PresetDetailScreenProps) {
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)' }}>
      ...
      <div style={{
        ...
        <div style={{
          marginTop: 'var(--seed-dimension-x3_5)', display: 'flex', gap: 'var(--seed-dimension-x4_5)',
          fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)',
        }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x1)' }}>
            <I n="map-pin" s={12} c="var(--seed-color-palette-static-white)" />
            <span>{preset.placeIds.length}곳</span>
          </span>
          {preset.savedCount != null && (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x1)' }}>
              <I n="bookmark" s={12} c="var(--seed-color-palette-static-white)" />
      ...
      {preset.ownerName && !isMine && (
        <div style={{ padding: 'var(--seed-dimension-x4) var(--seed-dimension-spacing-x-global-gutter) 0' }}>
          <ListRow
            leading={<Avatar initial={preset.ownerInitial} size={40} color={preset.ownerColor || 'var(--seed-color-bg-brand-solid)'} />}
      ...
      {preset.tags && preset.tags.length > 0 && (
        <div style={{ padding: 'var(--seed-dimension-x4) var(--seed-dimension-spacing-x-global-gutter) 0' }}>
          <div style={{ display: 'flex', gap: 'var(--seed-dimension-x1_5)', flexWrap: 'wrap' }}>
            {preset.tags.map(t => (
      ...
      <SectionHeader title="코스 순서" subtitle={`${preset.placeIds.length}곳`}
        trailing={
          <div style={{
            display: 'inline-flex', background: 'var(--seed-color-bg-neutral-weak)',
            borderRadius: 'var(--seed-radius-r3)', padding: 'var(--seed-dimension-x1)',
          }}>
            {[
              { id: 'list', label: '리스트' },
              { id: 'map', label: '지도' },
              ...
              <button key={t.id} onClick={() => setView(t.id)} style={{
                padding: 'var(--seed-dimension-x1_5) var(--seed-dimension-x3)', borderRadius: 'var(--seed-radius-r2)',
                background: view === t.id ? 'var(--seed-color-palette-static-white)' : 'transparent',
                color: view === t.id ? 'var(--seed-color-fg-neutral)' : 'var(--seed-color-fg-neutral-subtle)',
                border: 'none', cursor: 'pointer',
                fontWeight: 'var(--seed-font-weight-bold)', fontSize: 'var(--seed-font-size-t2)',
              }}>{t.label}</button>
            ))}
      ...
      {view === 'list' && (
        <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)', display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x2_5)' }}>
          {preset.placeIds.map((pid, i) => {
            ...
            return (
              <Card key={pid} padding={0} radius={14} onClick={() => onOpenPlace?.(pid)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3)', padding: 'var(--seed-dimension-x3)' }}>
                  <div style={{
                    width: 'var(--seed-dimension-x7)', height: 'var(--seed-dimension-x7)', borderRadius: '50%',
                    background: grp.color, color: 'var(--seed-color-palette-static-white)',
                    display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-bold)', fontVariantNumeric: 'tabular-nums', flexShrink: 0,
                  }}>{i + 1}</div>
                  <div style={{
                    width: 'var(--seed-dimension-x13)', height: 'var(--seed-dimension-x13)', borderRadius: 'var(--seed-radius-r3)', overflow: 'hidden', flexShrink: 0,
                    border: '1px solid var(--seed-color-stroke-neutral-subtle)',
                  }}>
                    <PhotoPlaceholder hue={p.hue} lift={p.lift}
                      style={{ width: '100%', height: '100%' }} />
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{
                      fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', marginTop: 'var(--seed-dimension-x0_5)',
                      color: grp.color, display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x1)',
                    }}>
      ...
      {!isMine && (
        <div style={{ padding: 'var(--seed-dimension-x6) var(--seed-dimension-spacing-x-global-gutter) 0' }}>
          <Button variant={isSaved ? 'soft' : 'primary'} size="large" fullWidth
            onClick={togglesSave}
            leading={<I n={isSaved ? 'check' : 'bookmark'} s={16} c={isSaved ? 'var(--seed-color-fg-neutral)' : 'var(--seed-color-palette-static-white)'} w={2.2} />}>
...
// ─────────────────────────────────────────────
function PresetCreateScreen({ regionId, onBack, onCreated }: PresetCreateScreenProps) {
  const region = getRegion(regionId);
  const [title, setTitle] = React.useState('');
  const [description, setDescription] = React.useState('');
  const [order, setOrder] = React.useState<string[]>([]); // ordered list of placeIds
  ...
  const save = () => {
    ...
    const preset: Preset = {
      ...
      ownerName: '나',
      ownerInitial: '나',
      ownerColor: 'var(--seed-color-bg-brand-solid)',
      region: regionId,
      title: title.trim(),
    ...
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)' }}>
      ...
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-x4)' }}>
        <Card padding={12} radius={12} style={{
          ...
          <div style={{
            fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-muted)',
            lineHeight: 1.55, }}>한 프리셋에는 같은 지역의 자원만 담을 수 있어요. 발견한 자원 중에서만 선택할 수 있습니다.</div>
        </Card>
      </div>
      ...
      <div style={{ padding: '0 var(--seed-dimension-x5) var(--seed-dimension-x3)' }}>
        <div style={{
          display: 'flex', flexWrap: 'wrap', gap: 'var(--seed-dimension-x1_5)', marginBottom: 'var(--seed-dimension-x2)',
        }}>
          {tags.map(t => (
            <Chip key={t} active onClick={() => removeTag(t)}
              trailing={<I n="close" s={11} c="var(--seed-color-palette-static-white)" w={2.5} />}
              size="small">
              #{t}
            </Chip>
          ))}
        ...
        {tags.length === 0 && (
          <div style={{
            marginTop: 'var(--seed-dimension-x2)', display: 'flex', gap: 'var(--seed-dimension-x1)', flexWrap: 'wrap',
          }}>
      ...
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)', display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x2)' }}>
        {order.length === 0 && (
          <div style={{
            padding: 'var(--seed-dimension-x6) var(--seed-dimension-x4)', borderRadius: 'var(--seed-radius-r4)',
            background: 'var(--seed-color-bg-layer-basement)', border: '1px dashed var(--seed-color-palette-gray-500)',
            fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-muted)',
            textAlign: 'center', }}>아래에서 발견한 자원을 골라 추가하세요</div>
        )}
        {order.map((pid, i) => {
          const p = PLACES.find(x => x.id === pid);
          if (!p) return null;
          ...
          return (
            <Card key={pid} padding={10} radius={12}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x2_5)' }}>
                <span style={{
                  width: 'var(--seed-dimension-x6)', height: 'var(--seed-dimension-x6)', borderRadius: '50%',
                  background: grp.color, color: 'var(--seed-color-palette-static-white)',
                  display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', fontVariantNumeric: 'tabular-nums', flexShrink: 0,
                }}>{i + 1}</span>
                <div style={{
                  width: 'var(--seed-dimension-x10)', height: 'var(--seed-dimension-x10)', borderRadius: 'var(--seed-radius-r2)', overflow: 'hidden', flexShrink: 0,
                }}>
                  <PhotoPlaceholder hue={p.hue} lift={p.lift}
                    style={{ width: '100%', height: '100%' }} />
                </div>
                ...
                <div style={{ display: 'flex', gap: 'var(--seed-dimension-x1)' }}>
                  <IconButton size="small" variant="soft" onClick={() => move(pid, -1)} ariaLabel="위로">
                    <I n="chevron-up" s={16} c="var(--seed-color-fg-neutral)" />
                  </IconButton>
                  <IconButton size="small" variant="soft" onClick={() => move(pid, 1)} ariaLabel="아래로">
                    <I n="chevron-down" s={16} c="var(--seed-color-fg-neutral)" w={2} />
                  </IconButton>
                  <IconButton size="small" variant="soft" onClick={() => toggle(pid)} ariaLabel="제거">
                    <I n="close" s={16} c="var(--seed-color-fg-critical)" w={2} />
                  </IconButton>
                </div>
              </div>
            </Card>
        ...
        {candidates.map(p => {
          ...
          return (
            <Card key={p.id} padding={0} radius={12} onClick={() => toggle(p.id)}
              style={{
                border: selected ? `2px solid ${grp.color}` : '1px solid var(--seed-color-stroke-neutral-subtle)',
              }}>
              <div style={{
                ...
                {selected && (
                  <div style={{
                    ...
                    <div style={{
                      width: 'var(--seed-dimension-x8)', height: 'var(--seed-dimension-x8)', borderRadius: '50%',
                      background: 'var(--seed-color-palette-static-white)',
                      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: 'var(--seed-font-size-t4)', fontWeight: 'var(--seed-font-weight-bold)', color: grp.color,
                      fontVariantNumeric: 'tabular-nums',
              ...
              <div style={{ padding: 'var(--seed-dimension-x2_5)' }}>
                ...
                <div style={{
                  fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: grp.color,
                  marginTop: 'var(--seed-dimension-x0_5)', display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x1)',
                }}>
...
function userCollectionStats(userId: string): UserCollectionStats {
  let h = 0;
  ...
  for (const p of ownerPresets) {
    ...
    if (region) {
      perRegion[p.region] = Math.min(region.total, Math.max(perRegion[p.region] || 0, p.placeIds.length + 2));
    }
  }
...
// ─────────────────────────────────────────────
function userTitles(userId: string): CollectionTitle[] {
  let h = 0;
  for (let i = 0; i < userId.length; i++) h = (h * 31 + userId.charCodeAt(i)) | 0;
...
// Other User Profile
// ─────────────────────────────────────────────
function UserProfileScreen({ userId, onBack, onOpenPreset, onOpenRegion }: UserProfileScreenProps) {
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)' }}>
      <div style={{
        ...
        <div style={{ marginTop: 'var(--seed-dimension-x5)', display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x3_5)' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{
              fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-medium)',
              color: 'var(--seed-color-palette-static-white-alpha-600)', marginTop: 'var(--seed-dimension-x1)',
        ...
        <div style={{
          marginTop: 'var(--seed-dimension-x4_5)',
          display: 'flex', alignItems: 'stretch',
          background: 'var(--seed-color-palette-static-white-alpha-50)',
          borderRadius: 'var(--seed-radius-r4)', padding: 'var(--seed-dimension-x3_5) var(--seed-dimension-x1)',
        }}>
      ...
      <div style={{
        display: 'flex', gap: 'var(--seed-dimension-x2)', overflowX: 'auto', padding: '0 var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-x1)',
        scrollbarWidth: 'none', msOverflowStyle: 'none',
      }}>
        {ownedTitles.map(t => {
          const tier = TITLE_TIERS[t.tier];
          return (
            <div key={t.id} style={{
              ...
              <span style={{
                width: 'var(--seed-dimension-x7)', height: 'var(--seed-dimension-x7)', borderRadius: 'var(--seed-radius-r2)',
...
function UserDexCarousel({ filled, empty, onOpenRegion }: UserDexCarouselProps) {
  ...
  return (
    <>
      ...
      {empty.length > 0 && (
        <div style={{
          padding: 'var(--seed-dimension-x2_5) var(--seed-dimension-x5) 0',
          display: 'flex', flexWrap: 'wrap', gap: 'var(--seed-dimension-x1_5)',
        }}>
          <span style={{
            fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)',
            marginRight: 'var(--seed-dimension-x1)', marginTop: 'var(--seed-dimension-x1)',
          }}>아직:</span>
          {empty.map(({ region }) => (
...
function ForeignRegionMini({ region, collected, onClick }: ForeignRegionMiniProps) {
  const pct = region.total ? collected / region.total : 0;
  return (
    <Card padding={10} radius={12} onClick={onClick}>
      <div style={{
        width: '100%', aspectRatio: '1', borderRadius: 'var(--seed-radius-r3)',
        background: 'var(--seed-color-bg-layer-basement)', padding: 'var(--seed-dimension-x1)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
      ...
      <div style={{
        marginTop: 'var(--seed-dimension-x2)', fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral)',
        overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
      }}>{region.name}</div>
      <div style={{
        display: 'flex', alignItems: 'baseline', justifyContent: 'space-between',
...
function UserStatColumn({ value, unit, label }: StatColumnProps) {
  return (
    <div style={{
      flex: 1, padding: '0 var(--seed-dimension-x2)', textAlign: 'center',
      display: 'flex', flexDirection: 'column', justifyContent: 'center',
    }}>
      <div style={{
        fontSize: 'var(--seed-font-size-t7)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-static-white)',
        lineHeight: 1,
        fontVariantNumeric: 'tabular-nums',
      }}>
        {value}
        {unit && <span style={{
          fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)',
          color: 'var(--seed-color-palette-static-white-alpha-600)', marginLeft: 'var(--seed-dimension-x0_5)',
        }}>{unit}</span>}
      </div>
      <div style={{
        fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)',
        color: 'var(--seed-color-palette-static-white-alpha-700)', marginTop: 'var(--seed-dimension-x1_5)',
        }}>{label}</div>
    </div>
  );
}
...
function ProfileScreen({ user, onOpenTitles, onReset, onOpenPreset }: ProfileScreenProps) {
  const collected = PLACES.filter(p => p.collected).length;
  const visitedRegions = REGIONS.filter(r => r.collected > 0).length;
  ...
  return (
    <div style={{ paddingBottom: 'calc(var(--seed-dimension-spacing-y-screen-bottom) + var(--seed-dimension-x14))', background: 'var(--seed-color-palette-static-white)' }}>
      ...
      <div style={{
        background: 'var(--seed-color-bg-neutral-inverted)', color: 'var(--seed-color-fg-neutral-inverted)',
        padding: 'var(--seed-dimension-x14) var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-x8)',
      }}>
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          ...
          <div style={{
            fontSize: 'var(--seed-font-size-t1)', fontWeight: 'var(--seed-font-weight-bold)',
            color: 'var(--seed-color-palette-static-white-alpha-600)', }}>나의 발견</div>
          <IconButton variant="inverse" ariaLabel="설정">
            <I n="settings" s={18} c="var(--seed-color-palette-static-white)" />
          </IconButton>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x4)', marginBottom: 'var(--seed-dimension-x6)' }}>
          <Avatar initial={(user?.name || '꼬마탐험가')[0]} size={72} color="var(--seed-color-bg-brand-solid)"
            ring="var(--seed-color-palette-static-white-alpha-300)" />
          <div style={{ flex: 1 }}>
            <div style={{
              fontSize: 'var(--seed-font-size-t9)',
              lineHeight: 'var(--seed-line-height-t9)',
            ...
            {equippedTitle && (
              <div style={{
                ...
                display: 'inline-flex', alignItems: 'center', gap: 'var(--seed-dimension-x1_5)',
                padding: 'var(--seed-dimension-x1) var(--seed-dimension-x2_5)', borderRadius: 'var(--seed-radius-full)',
                background: 'var(--seed-color-palette-static-white-alpha-100)',
                fontSize: 'var(--seed-font-size-t2)', fontWeight: 'var(--seed-font-weight-bold)',
                }}>
                <I n={equippedTitle.icon} s={12} c="var(--seed-color-palette-static-white)" w={2} />
      ...
      <div style={{ padding: 'var(--seed-dimension-x5) var(--seed-dimension-spacing-x-global-gutter) 0' }}>
        <Card padding={0} radius={16}>
          <ListRow
            leading={<span style={{
              width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x9)', borderRadius: 'var(--seed-radius-r3)', background: 'var(--seed-color-palette-yellow-100)',
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            }}><I n="trophy" s={18} c="var(--seed-color-palette-yellow-700)" w={2} /></span>}
            title="칭호" subtitle="5티어 시스템"
            trailing={
              <>
                <span style={{
                  fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)',
                  marginRight: 'var(--seed-dimension-x1_5)', fontVariantNumeric: 'tabular-nums',
                }}>{earnedTitles}개</span>
                <I n="chevron-right" s={16} c="var(--seed-color-fg-neutral-subtle)" />
              </>
            }
          ...
          <ListRow
            leading={<span style={{
              width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x9)', borderRadius: 'var(--seed-radius-r3)', background: 'var(--seed-color-bg-brand-weak)',
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            }}><I n="bookmark" s={18} c="var(--seed-color-fg-brand)" w={2} /></span>}
            title="가고싶다 저장"
            trailing={
              <>
                <span style={{
          ...
          <ListRow
            leading={<span style={{
              width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x9)', borderRadius: 'var(--seed-radius-r3)', background: 'var(--seed-color-palette-purple-200)',
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            }}><I n="calendar" s={18} c="var(--seed-color-palette-purple-700)" w={2} /></span>}
            title="여행 기록"
            trailing={<I n="chevron-right" s={16} c="var(--seed-color-fg-neutral-subtle)" />}
          />
          <ListRow
            leading={<span style={{
              width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x9)', borderRadius: 'var(--seed-radius-r3)', background: 'var(--seed-color-palette-green-200)',
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            }}><I n="gift" s={18} c="var(--seed-color-palette-green-700)" w={2} /></span>}
            title="협력 혜택"
            trailing={
              <>
                <Badge tone="danger" variant="solid" size="xsmall">3</Badge>
                <I n="chevron-right" s={16} c="var(--seed-color-fg-neutral-subtle)" style={{ marginLeft: 'var(--seed-dimension-x1_5)' }} />
              </>
            }
          />
          <ListRow
            leading={<span style={{
              width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x9)', borderRadius: 'var(--seed-radius-r3)', background: 'var(--seed-color-bg-neutral-weak)',
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            }}><I n="globe" s={18} c="var(--seed-color-fg-neutral-muted)" w={2} /></span>}
            title="언어"
            trailing={
              <>
                <span style={{
                  fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-fg-neutral-subtle)',
                  marginRight: 'var(--seed-dimension-x1_5)',
                }}>한국어 · +8</span>
                <I n="chevron-right" s={16} c="var(--seed-color-fg-neutral-subtle)" />
              </>
            }
          ...
          <ListRow
            leading={<span style={{
              width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x9)', borderRadius: 'var(--seed-radius-r3)', background: 'var(--seed-color-bg-critical-weak)',
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
            }}><I n="refresh" s={18} c="var(--seed-color-fg-critical-contrast)" w={2} /></span>}
            title="데모 초기화"
            trailing={<I n="chevron-right" s={16} c="var(--seed-color-fg-neutral-subtle)" />}
            onClick={onReset}
            last
      ...
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)', display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x2_5)' }}>
        {myPresets.length === 0 ? (
          <Card padding={16} radius={14} style={{
            ...
            <div style={{
              fontSize: 'var(--seed-font-size-t3)', fontWeight: 'var(--seed-font-weight-medium)', color: 'var(--seed-color-fg-neutral-subtle)',
              lineHeight: 1.55, }}>아직 등록한 프리셋이 없어요.<br/>도감 &gt; 지역 상세에서 코스를 만들어보세요.</div>
          </Card>
        ) : (
          myPresets.map(p => (
      ...
      <div style={{ padding: '0 var(--seed-dimension-spacing-x-global-gutter)', display: 'flex', flexDirection: 'column', gap: 'var(--seed-dimension-x2)' }}>
        {catBreak.map(c => (
          <Card key={c.group.id} padding={14} radius={14}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--seed-dimension-x2_5)', marginBottom: 'var(--seed-dimension-x2)' }}>
              <span style={{
                width: 'var(--seed-dimension-x9)', height: 'var(--seed-dimension-x9)', borderRadius: 'var(--seed-radius-r3)',
              ...
              <div style={{
                fontSize: 'var(--seed-font-size-t6)', fontWeight: 'var(--seed-font-weight-bold)', color: c.group.color,
                fontVariantNumeric: 'tabular-nums',
      ...
      <div style={{
        marginTop: 'var(--seed-dimension-x6)', padding: '0 var(--seed-dimension-spacing-x-global-gutter) var(--seed-dimension-x5)', textAlign: 'center',
      }}>
        <Brand size={18} color="var(--seed-color-fg-neutral-subtle)" />
      </div>
    </div>
  );
}

function ProfileStat({ value, unit, label }: StatColumnProps) {
  return (
    <div style={{
      flex: 1, textAlign: 'center', padding: '0 var(--seed-dimension-x1_5)',
    }}>
      <div style={{
        fontSize: 'var(--seed-font-size-t8)', fontWeight: 'var(--seed-font-weight-bold)', color: 'var(--seed-color-palette-static-white)',
        lineHeight: 1,
