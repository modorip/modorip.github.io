...
// src/lib/screens.js의 현행 13개 화면(번호/이름/순서)에 1:1 정렬.
// 각 화면은 실제 design/bundle.jsx 컴포넌트 구조를 저충실도(박스+레이블)로 표현한다.
// 디바이스 비율 402×874 (MockFrame CANVAS_W/H) · seed-design CSS 토큰 사용.

const CAT = ['var(--wf-nature)', 'var(--wf-heritage)', 'var(--wf-culture)', 'var(--wf-festival)'];
const CATN = ['자연', '유적', '문화', '축제'];

// ── low-fi 블록 빌더 ─────────────────────────────────────────────
const R = {
  // 텍스트 박스 (멀티라인 \n 지원)
  box: (l, h = 40) => `<div class="b box" style="min-height:${h}px">${l || ''}</div>`,
  // 이미지/사진 placeholder
  img: (l, h = 80) => `<div class="b box img" style="min-height:${h}px">${l || '이미지'}</div>`,
  // 앱 헤더 바 (브랜드 + 우측 아이콘)
  bar: (l) => `<div class="b bar">${l}<span class="icns"><span class="ic"></span><span class="ic"></span></span></div>`,
  // 헤더 (서브 + 타이틀 + 트레일링)
  hdr: (s, t, tr) => `<div class="b hdr">${s ? `<div class="sub">${s}</div>` : ''}<div class="ttl">${t}${tr ? `<span class="tr">${tr}</span>` : ''}</div></div>`,
  // 통계 스트립
  ...
  cat: (fill = [1, 0, 0, 0]) => `<div class="b cat">${CAT.map((c, i) => `<div class="cb" style="background:${fill[i] ? c : 'var(--wf-soft)'};color:${fill[i] ? '#fff' : 'var(--wf-sub)'}">${CATN[i]}</div>`).join('')}</div>`,
  // 메모/주석
  ...
  prog: (p, light = false) => `<div class="prog${light ? ' light' : ''}"><i style="width:${p}%"></i></div>`,
  // 가로 동등 분할 row
  row: (items) => `<div class="b row">${items.join('')}</div>`,
  // 리스트 행 (아이콘 + 제목/부제 + 우측)
  listrow: (t, s, tr = '') => `<div class="listrow"><span class="ld"></span><span class="tx"><span class="t">${t}</span><span class="s">${s}</span></span>${tr ? `<span class="tr">${tr}</span>` : ''}</div>`,
  // 아바타 행 (작성자 카드)
  avrow: (nm, sub, tr = '') => `<div class="listrow"><span class="av">${nm[0]}</span><span class="tx"><span class="t">${nm}</span><span class="s">${sub}</span></span>${tr ? `<span class="tr">${tr}</span>` : ''}</div>`,
  // 그라데이션 히어로 (tone=지역 블루 톤)
  hero: (small, big, tone = false, extra = '') => `<div class="b hero${tone ? ' tone' : ''}">${small ? `<div class="small">${small}</div>` : ''}<div class="big">${big}</div>${extra}</div>`,
  // 도감 그리드
  ...
  cell: (idx, label, locked = false, badge = false) => `<div class="cell${locked ? ' locked' : ''}"><div class="ci">${idx ? `<span class="idx">${idx}</span>` : ''}${badge ? '<span class="bdg"></span>' : ''}<span class="cph">사진</span></div><div class="ct">${label}</div></div>`,
};
...
const S = [
  ...
  // 03 발견 (GPS 지도)
  {
    ...
    custom:
      ...
      `<span class="pin" style="left:36%;top:28%;background:var(--wf-nature)"></span>` +
      `<span class="pin sel" style="left:58%;top:46%"></span>` +
      `<span class="pin" style="left:48%;top:64%;background:var(--wf-festival)"></span>` +
  ...
  // 07 장소 상세
  {
    ...
    scroll: [
      ...
      R.sect('30일 혼잡도'),
      R.grid(10, Array.from({ length: 10 }, (_, i) => `<div class="cell heat"><div class="ci" style="background:${i === 3 ? 'var(--wf-busy)' : (i % 3 ? 'var(--wf-mid)' : 'var(--wf-calm)')}"></div></div>`)),
  ...
  // 13 프리셋 — 상세
  {
    ...
    scroll: [
      R.hero('강원', '강원 일출 코스', true, `<div class="small" style="margin-top:6px">자원 5곳 · 담음 320회</div>`),
      R.avrow('작성자', '등록 12 · 퍼감 540', '›'),
      R.box('설명 · 멀티라인 본문', 42),
